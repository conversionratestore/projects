(function() {
  ...
  const c = (e, n, o, a = "") => {
    ...
    let n = setInterval(function() {
      typeof window.clarity == "function" && (clearInterval(n), window.clarity("set", e, "variant_2"));
    }, 1e3);
  };
  function i(e) {
...
.note_block p {
  ...
  class x {
    ...
    init() {
      p({ name: "select Trial Price Page Improvements V2", dev: "SKh" }), d("exp_select_trial"), c(
        "exp_select_trial_activated_c",
    ...
    handleSubscriptionPage() {
      ...
      ), t(".crs_style") || document.head.insertAdjacentHTML("beforeend", `<style class="crs_style">${u}</style>`), this.renderTitleBlock(), this.renderEmailBlock(), this.renderNoteBlock(), this.renderProceedToFindOutBlock(), this.renderGuaranteeBlock(), this.replaceTxtBtnGetStarted();
    }
    renderTitleBlock() {
