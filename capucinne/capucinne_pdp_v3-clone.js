...
let startFunkFastNav = setInterval(() => {
  ...
  let newStyle = /*html */ `
    <style>
      ...
      .lav-fast-toggler .tippy-tooltip {
        border-radius: 1px;
        border: 1px solid #e8e8e1;
        background: #fff;
        color: #000;
      ...
      .lav-fast-toggler .tippy-popper {
        max-width: 135px !important;
        left: 10px !important;
      }
      .lav-fast-toggler .tippy-tooltip[data-placement^="bottom"] > .tippy-arrow {
        border-bottom-color: #fff;
      }
      .lav-fast-toggler .tippy-tooltip[data-placement^="top"] > .tippy-arrow {
        border-top-color: #fff;
      }
      .lav-fast-toggler .tippy-content {
        padding: 4px 7px;
      }
      .switch_title {
        margin: 0;
      ...
      .lav-toggler-wrap .lav-fast-toggler {
        margin-right: 30px;
      }
      @media (max-width: 992px) {
        ...
        .collection-single__description#moreText {
          margin-bottom: 15px;
        }
        #btn_read_more {
          display: inline-block;
        }
        .collection-single__description a.btn + h2 {
          margin-top: 15px;
        }
        .lav-toggler-wrap .lav-fast-toggler {
          position: absolute;
  ...
  function renderNewElem() {
    ...
    if (document.querySelector(".collection-filter__item.collection-filter__item--sort") && !document.querySelector(".lav-fast-toggler") && location.href.includes('/collections/') && !location.href.includes('collections/fast-production')) {
      if (!document.querySelector('#btn_read_more')) {
        document.querySelector('.collection-single__description').classList.add('lav-description');
      }
      let fasterAvailabilityBlock = /*html */ `
        <div class='lav-fast-toggler'>
          <div class="switch_btn">
            <input type="checkbox" name="properties[fasterAvailability]" value="" id='switchLabel-1' class="switch_btn_checkbox" />
            <label for="switchLabel-1" class="switch_btn_label"></label>
          </div>
          <p class="switch_title">
            Faster Availability
            <svg class="tooltip_toggle" data-tooltip data-title="Faster than standard production time" width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="7.87793" cy="7.70068" r="6.5" fill="white" stroke="#D4A298"></circle>
              <path d="M7.88724 5.70868C7.72191 5.70868 7.58324 5.65268 7.47124 5.54068C7.35924 5.42868 7.30324 5.29002 7.30324 5.12468C7.30324 4.95935 7.35924 4.82068 7.47124 4.70868C7.58324 4.59668 7.72191 4.54068 7.88724 4.54068C8.04724 4.54068 8.18324 4.59668 8.29524 4.70868C8.40724 4.82068 8.46324 4.95935 8.46324 5.12468C8.46324 5.29002 8.40724 5.42868 8.29524 5.54068C8.18324 5.65268 8.04724 5.70868 7.88724 5.70868ZM8.33524 6.29268V10.7007H7.42324V6.29268H8.33524Z" fill="#D4A298"></path>
            </svg>
          </p>
      ...
      let observer = new MutationObserver((mutations) => {
        for (let mutation of mutations) {
            for (let node of mutation.addedNodes) {
              ...
              if (!node.classList.contains('page-width') || !node.closest('#CollectionAjaxContent') || document.querySelector('.lav-fast-toggler')) continue;
      ...
      function addToggers() {
        if (location.href.includes('filter.p.tag=Faster+availability')) {
          if (!fasterAvailabilityBlock.includes('checked="checked"')) {
            fasterAvailabilityBlock = fasterAvailabilityBlock.replace('type="checkbox"', 'type="checkbox" checked="checked"');
          }
        } else {
          fasterAvailabilityBlock = fasterAvailabilityBlock.replace('checked="checked"', '');
        }

        document.querySelector(".collection-filter__item.collection-filter__item--sort").classList.add('lav-toggler-wrap');
        document.querySelector(".lav-toggler-wrap")?.insertAdjacentHTML("afterbegin", fasterAvailabilityBlock);
            
        document.querySelector("#filter")?.insertAdjacentHTML("afterend", fasterAvailabilityBlock.replaceAll('switchLabel-1', 'switchLabel-2'));

        document.querySelectorAll('.lav-fast-toggler .switch_btn input').forEach(item => {
          item.addEventListener('change', function () {
            const isNavbar = this.closest('#FilterDrawer') ? true : false;
            console.log('this.checked', this.checked)
            toggleFilter(this.checked, isNavbar);
          });
        })
      }
  

      function toggleFilter(flag, isNavbar) {
        const isActive = document.querySelector('.filter-form [value="Faster availability"]').checked;
        if (isNavbar) {
          pushDataLayer(['exp_fast_prod_nav_rad_listfilt_avail', `${isActive ? 'SetActive' : 'SetInactive'} - Click`, 'Radio button', 'Listing Filter']);
        } else {
          pushDataLayer(['exp_fast_prod_nav_rad_listing_avail', `${isActive ? 'SetActive' : 'SetInactive'} - Click`, 'Radio button', 'Listing']);
        }
  ...
  function tooltipInit(onUpdate = false) {
    if (!onUpdate) {
    ...
    let tippyRunListing = setInterval(() => {
      if (typeof tippy === "function" && document.querySelector(".lav-fast-toggler [data-tooltip]")) {
        clearInterval(tippyRunListing);
        document.querySelectorAll(".lav-fast-toggler [data-title]").forEach((el) => {
          let time = 0;
          let timer;
          tippy(el, {
            content: el.getAttribute("data-title"),
            ...
            onShow(e) {
              ...
              // TODO - make it work after update
              if (el.closest('#FilterDrawer')) {
                pushDataLayer(['exp_fast_prod_nav_tip_listfilt_fast', 'Faster than standard production time', 'Tips', 'Listing Filter']);
              } else {
                pushDataLayer(['exp_fast_prod_nav_tip_listing_fat', 'Faster than standard production time', 'Tips', 'Listing']);
              }
            },
            onHide(e) {
              clearInterval(timer);
              if (el.closest('#FilterDrawer')) {
                pushDataLayer(['exp_fast_prod_nav_vis_listfilt_foc', time + 's', 'Visibility ', 'Listing Filter']);
              } else {
                pushDataLayer(['exp_fast_prod_nav_vis_list_foc', time + 's', 'Visibility ', 'Listing']);
              }
            }
          });
        });
