...
`;
  class z {
    ...
    `
      ...
      document.head.insertAdjacentHTML("afterbegin", `<style>${L}</style>`), e == null || e.insertAdjacentHTML("beforeend", n), i == null || i.insertAdjacentHTML("beforeend", t);
    }
    update(n) {
      if (n < 1 || n > 4) {
...
.os-subnote .os-saveup-note .icon {
  ...
  const Z = "https://api.sunvalue.com/api/address?zip=", F = "https://api.sunvalue.com/api/geo";
  async function w(r, n = "GET", t = null) {
    try {
      const e = {
        method: n,
        headers: {
  ...
  const D = async (r) => await w(Z + r), V = async () => await w(F), W = `#estimate-zip h1:not(.os-title) {
    ...
    addInfoBlock() {
      ...
      <div class="os-info-block">
        ...
        <ul class="os-info-block__list">
          <li><span>${x}</span>Location</li>
          <li><span>${$}</span>Your energy bill amount</li>
          <li><span>${M}</span>Best incentives provided in your area</li>
        </ul>
      </div>
    `
    ...
    async titleAndZipChanges() {
      ...
      if (a && (a.textContent = ""), e) {
        const l = (
          /* HTML */
          `<h1 class="os-title">
        See if your home qualifies for <span>${n.city} Solar Energy Program</span>
      </h1>`
        );
      ...
      if (i && (i.textContent = "Enter your ZIP code to get started"), o && (o.textContent = "GeT STARTED", o.addEventListener("click", () => {
        ...
        <div class="os-current-location">
          <span class="os-current-location__icon">${x}</span>
...
.os-incentive-note .icon {
  ...
  const X = `.estimate-custom-homeowner .os-title {
  font-size: 32px;
  font-weight: 900;
  line-height: 42px;
  text-align: left;
...
`;
  ...
  const K = `.estimate-custom-name .os-title {
  display: flex;
  align-items: center;
  gap: 12px;
  font-size: 32px;
...
`;
  ...
  const nn = `.estimate-custom-email .os-title {
  display: flex;
  align-items: center;
  gap: 12px;
  font-size: 32px;
...
`;
  ...
  const en = `#estimate-phone .medium-container {
  max-width: 580px;
}
...
@media screen and (max-width: 768px) {
  ...
  class on {
    ...
    init() {
      document.head.insertAdjacentHTML("beforeend", `<style>${en}</style>`), this.addLabel(), this.inputChanges();
    }
    inputChanges() {
  ...
  const sn = `.swiper-container-android .swiper-slide,
.swiper-wrapper {
...
`;
  ...
  class rn {
    constructor() {
      var n;
      this.swiperElement = document.querySelector(".swiper-container"), this.swiper = (n = this.swiperElement) == null ? void 0 : n.swiper, this.progressBar = new z(), this.addNavigation(), this.otherEvents();
    }
    addNavigation() {
...
`;
  S({ name: "Hyp. #6 - Introduce short funnel with merging zip code with address step", dev: "OS" }), k("");
  class dn {
    constructor() {
