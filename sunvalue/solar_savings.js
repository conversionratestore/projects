console.log(
  '%c EXP: Sunvalue: Introduce solar savings and build trust through the funnel (DEV: SKh)',
...
const icons = {
  flashCircle: /* HTML */ `
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        d="M11.9697 2C6.44973 2 1.96973 6.48 1.96973 12C1.96973 17.52 6.44973 22 11.9697 22C17.4897 22 21.9697 17.52 21.9697 12C21.9697 6.48 17.4997 2 11.9697 2ZM15.7197 12.35L11.9997 16.58L11.5597 17.08C10.9497 17.77 10.4497 17.59 10.4497 16.66V12.7H8.74973C7.97973 12.7 7.76973 12.23 8.27973 11.65L11.9997 7.42L12.4397 6.92C13.0497 6.23 13.5497 6.41 13.5497 7.34V11.3H15.2497C16.0197 11.3 16.2297 11.77 15.7197 12.35Z"
        fill="#FB7306"
      />
    </svg>
  `,
...
const arrSliderReviewsMob = {
  1: [
    `${icons.stars}`,
    "We love our SunPower Solar. We had an extremely rainy winter and we still didn't nave a bill at the end of our year. Everyone associated with SunPower went above and beyond to be sure we were satisfied with our new system.",
    `${git}sunvalue/img/sunpower.png`
  ],
  2: [
    `${icons.stars}`,
    'Our experience with the techs was very, very positive. It went very smoothly. We had a very professional crew take care of all details and they not only installed the panels, but also installed a brand new electrical panel as well. We just got our first electric bill from our electrical provider and it was -$45 so everything that SunRun said was true. We are now producing more electricity than we are using.',
    `${git}sunvalue/img/sunrun.png`
  ],
  3: [
    `${icons.stars}`,
    'They followed through with everything and completed the installation on time. They call ahead of time and let you know what you want to do each step of the way. My electric bill has been reduced so it seems to be working. They are saving me money.',
    `${git}sunvalue/img/momentum_solar.png`
  ]
}
...
class changeFlow {
  ...
  init() {
    ...
    if (location.pathname === '/received/' || location.pathname === '/thankyou/') {
      ...
      checkVisibilityElem(
        '.crs_thank',
        'exp_intr_sol_sav_vis_thankyou_page',
        "You'll be contacted by a Solar Expert Partner in [City]  Thank you page"
      )
    }
  }
      ...
      <style>
        .powered_by_wrapper {
          ...
          background: linear-gradient(
            ...
            rgba(237, 242, 245, 0.8) 32.32%,
            rgba(237, 242, 245, 0.8) 66.9%,
    ...
    const html = /* HTML */ `
      <div class="federal_credit">
        ...
        ${this.lifetimeCostAnalysisHtml(
          ...
          'Before solar',
          'After solar*',
          'monthly_bill_comparison'
        )}
        ${this.lifetimeCostAnalysisHtml(
          'Lifetime cost  <br/> analysis with and without solar',
          'Lifetime cost*:',
          costWit,
          costWithout,
          toPrice,
        ...
        ${this.estimatedLifetimeSavingsHtml()}
      </div>
    `
    return html
  }
        ...
        <div class="monthly_bill_box">
          ...
          <div class="crs_graphic">
            <div class="crs_graphic_line_wrapp navy">
              ...
              <div
                class="crs_graphic_line navy"
                style="width: ${(100 * parseFloat(costWithout.replace(costWithout[0], '').split(',')[0])) / toPrice}%"
              ></div>
            </div>
            <div class="crs_graphic_line_wrapp green">
              ...
              <span>${costWit}</span>
              <div
                class="crs_graphic_line green"
                style="width: ${(100 * parseFloat(costWit.replace(costWit[0], '').split(',')[0])) / toPrice}%"
              ></div>
            </div>
          </div>
          ${this.poweredByHtml()}
        </div>
        <p class="footnote_txt">
          *These numbers are estimates according to our research from Project Sunroof research and may vary
        </p>
      </div>
    `
    return html
    ...
    waitForElement('.find_out_if_you_qualify_wrapper').then(el => {
      if (!$el('.find_out_if_you_qualify_wrapper + .monthly_bill_btns_wrapper.monthly_bill_comparison')) {
        el.insertAdjacentHTML(
          'afterend',
          this.monthlyBillBtnsBlockHtml(
            'Explore Lifetime Cost Analysis: Solar vs. Non-Solar',
            `${this.device === 'mobile' ? 'Next' : 'Explore Lifetime Cost Analysis'}`,
            'monthly_bill_comparison'
          )
        )
      }
    ...
    waitForElement('.find_out_if_you_qualify_wrapper').then(el => {
      if (!$el('.find_out_if_you_qualify_wrapper + .monthly_bill_btns_wrapper.lifetime_cost_analysis')) {
        el.insertAdjacentHTML(
          'afterend',
          this.monthlyBillBtnsBlockHtml(
            'View your lifetime solar savings<img src="https://conversionratestore.github.io/projects//sunvalue/img/money_icon.png" alt="money icon">',
            `${this.device === 'mobile' ? 'Next' : 'View your lifetime solar savings'}`,
            'lifetime_cost_analysis'
          )
        )
      }
      ...
      steps.forEach((element, index) => {
        ...
        if (element.querySelector('[id="estimate-zip"]')) {
          ...
          if (!element.querySelector('.no_personal_inform_wrapper')) {
            element
              .querySelector('.small-container')
              .insertAdjacentHTML('afterend', this.noPersonalInformationRequiredHtml())
          }
        ...
        if (element.querySelector('[id="estimate-bill"]')) {
          ...
          element
            .querySelector('.with-gradient .rangeslider__handle')
            .insertAdjacentElement('beforeend', element.querySelector('.rangeslider-tooltip'))
        }
        // estimate-map
        ...
        if (element.querySelector('[id="estimate-phone"]')) {
          ...
          if (!element.querySelector('.safe_and_secure_wrapper')) {
            element
              .querySelector('[for="leadid_tcpa_disclosure"]')
              .insertAdjacentHTML('afterend', this.safeAndSecureHtml())
          }
        }
      })
    })
    ...
    $('[data-rangeslider]').change(function (e) {
      if (changeSlider == 1) {
        if (!e.target.getAttribute('data-test')) {
          pushDataLayer(
            'exp_intr_sol_sav_range_enerbill_slider',
            'Slider',
            'Range slider',
            'How much was your latest monthly energy bill? 2nd step'
          )
        }
    ...
    if (city) {
      city.forEach(el => {
        if ($el('.incentive_list_custom').children.length !== city.length) {
          $el('.incentive_list_custom')?.insertAdjacentHTML(
            'beforeend',
            this.incentiveListCustomHtml(el.name, el.description)
          )
        }
      })
  ...
  initSliderReviews() {
    waitForElement('.reviews_box').then(el => {
      ...
      for (let key in arrSliderReviews) {
        $el('.slider_reviews_wrapper').insertAdjacentHTML(
          'beforeend',
          this.itemSliderReviewsHtml(arrSliderReviews[key][0], arrSliderReviews[key][1], arrSliderReviews[key][2])
        )
      }
      ...
      let s = setInterval(() => {
        if (typeof jQuery('.slider_reviews_wrapper').slick === 'function' && $el('.slider_reviews_wrapper')) {
          ...
          slider.on('swipe', function (event, slick, direction) {
            pushDataLayer(
              'exp_intr_sol_sav_but_houslocpup_swipe',
              direction,
              'Swipe slider',
              `Find your house location 4th step ${window.innerWidth < 769 ? 'Mobile' : 'Desktop'}`
            )
          })
          $$el('.slider_arrow').forEach(el => {
            el.addEventListener('click', i => {
              ...
              pushDataLayer(
                'exp_intr_sol_sav_but_houslocpup_arrow',
                arrowsTitle,
                'Button',
                `Find your house location 4th step ${window.innerWidth < 769 ? 'Mobile' : 'Desktop'}`
              )
  ...
  onClickYourSavingsBtn() {
    waitForElement('#calculateYourSavings').then(el => {
      el.addEventListener('click', e => {
        pushDataLayer(
          'exp_intr_sol_sav_but_solince_discov',
          'Discover Incentive Program',
          'Button',
          '[City] SolarIncentive Programs 1st step'
        )
    ...
    waitForElement('#slider-block .default').then(el => {
      el.addEventListener('click', e => {
        if ($('.swiper-slide').eq(2).hasClass('swiper-slide-active')) {
          pushDataLayer(
            'exp_intr_sol_sav_but_enerbill_next',
            'Next',
            'Button',
            'How much was your latest monthly energy bill? 2nd step'
          )
      ...
      $$el('[id="companies"] input').forEach(i => {
        i.addEventListener('click', e => {
          ...
          if (!localStorage.getItem('stepThree')) {
            pushDataLayer(
              'exp_intr_sol_sav_but_utilprov_watpow',
              txtInput,
              'Button',
              'Who is your utility provider? 3rd step'
            )
          }
        ...
        i.addEventListener('click', e => {
          if (!e.target.getAttribute('data-test')) {
            ...
            if (!localStorage.getItem('stepFive')) {
              pushDataLayer(
                `exp_intr_sol_sav_but_roofgetsun_${ev}`,
                txtInput,
                'Button',
                'Does your roof get sunlight? 5th step'
              )
            }
            ...
            if (crsData) {
              if (!$el('.federal_credit')) {
                $el('[id="estimate-email"]').insertAdjacentHTML(
                  'afterend',
                  this.searchingAvailableIncentiveProgramsHtml(crsData)
                )
              }
  ...
  onClickShowSolarSavingsBtn() {
    waitForElement('#showSolarSavingsBtn').then(el => {
      el.addEventListener('click', e => {
        pushDataLayer(
          'exp_intr_sol_sav_but_available_show',
          'Show solar savings',
          'Button',
          'Availableincentives found 7th step'
        )
        ...
        link.addEventListener('click', e => {
          if (!e.target.getAttribute('data-test')) {
            ...
            if (e.target.closest('.lifetime_cost_analysis')) {
              pushDataLayer(
                'exp_intr_sol_sav_but_lifeanalys_next',
                'Next',
                'Button',
                'Lifetime cost analysis with and without solar  9th step'
              )
  ...
  onClickfindOutIfYouQualifyBtn() {
    waitForElement('#findOutIfYouQualifyBtn').then(el => {
      el.addEventListener('click', e => {
        pushDataLayer(
          'exp_intr_sol_sav_but_estimat_find',
          'FIND OUT IF YOU QUALIFY',
          'Button',
          'Estimatedlifetime savings  10th step'
        )
  ...
  onClickOldBtnBack() {
    $el('.back-link').addEventListener('click', () => {
      $$el('.swiper-slide-next').forEach(el => {
        ...
        if (step.includes('2')) {
          pushDataLayer(
            'exp_intr_sol_sav_but_enerbill_back',
            'Back',
            'Button',
            'How much was your latest monthly energy bill? 2nd step'
          )
        ...
        if (step.includes('3')) {
          pushDataLayer(
            'exp_intr_sol_sav_but_utilprov_black',
            'Back',
            'Button',
            'Who is your utility provider? 3rd step'
          )
        ...
        if (step.includes('5')) {
          pushDataLayer(
            'exp_intr_sol_sav_but_roofgetsun_back',
            'Back',
            'Button',
            'Does your roof get sunlight? 5th step'
          )
      ...
      btn.addEventListener('click', e => {
        $$el('.swiper-slide-next').forEach(el => {
          ...
          if (step.includes('7') && e.target.closest('.lifetime_cost_analysis')) {
            pushDataLayer(
              'exp_intr_sol_sav_but_lifeanalys_back',
              'Back',
              'Button',
              'Lifetime cost analysis with and without solar  9th step'
            )
  ...
  onClickElemPushDataLayer() {
    $('#zip').change(function () {
      pushDataLayer(
        'exp_intr_sol_sav_inp_solince_zipcod',
        'Enter Your Zip Code',
        'Input',
        '[City] SolarIncentive Programs 1st step'
      )
    ...
    $('#autoaddress').change(function () {
      pushDataLayer(
        'exp_intr_sol_sav_inp_housloc_adress',
        'Enter Your Address',
        'Input',
        'Find your house location 4th step'
      )
    })
    ...
    $('.swiper-wrapper .swiper-slide:nth-child(8) input').change(function () {
      pushDataLayer(
        'exp_intr_sol_sav_inp_numbreach_mobil',
        'Enter your mobile phone number',
        'Input',
        'What is the best number to reach you at if you qualify?'
      )
    })
    $('#submit').click(function () {
      pushDataLayer(
        'exp_intr_sol_sav_but_numbreach_subm',
        'Submit',
        'Button',
        'What is the best number to reach you at if you qualify?'
      )
    ...
    $('#tcpa_label a').click(function (e) {
      pushDataLayer(
        'exp_intr_sol_sav_lin_numbreach_solar',
        '4 solar partners',
        'Link',
        'What is the best number to reach you at if you qualify?'
      )
  ...
  visibScreenView() {
    ...
    checkVisibilityElem(
      ...
      'exp_intr_sol_sav_vis_incenprog_page',
      '[City] SolarIncentive Programs 6th step'
    )
    checkVisibilityElem(
      '.incentive_wrapper',
      'exp_intr_sol_sav_vis_available_page',
      'Availableincentives found 7th step'
    )
    checkVisibilityElem(
      '.monthly_bill_comparison',
      'exp_intr_sol_sav_vis_comparis_page',
      'Monthlybill comparison 8th step'
    )
    checkVisibilityElem(
      '.lifetime_cost_analysis',
      'exp_intr_sol_sav_vis_lifeanalys_page',
      'Lifetime cost analysis with and without solar  9th step'
    )
    checkVisibilityElem(
      '.estimated_lifetime_sav_wrapper',
      'exp_intr_sol_sav_vis_estimat_page',
      'Estimated lifetime savings  10th step'
    )
  ...
  visibleSwiperSlideActivePage(i) {
    ...
    setTimeout(() => {
      if (viewedSlide != index && $('.swiper-slide').eq(index).hasClass('swiper-slide-active')) {
        ...
        if (index == 0) {
          pushDataLayer(
            'exp_intr_sol_sav_vis_solince_page',
            'Screen view',
            'Visibility ',
            '[City] SolarIncentive Programs 1st step'
          )
          ...
          pushDataLayer(
            'exp_intr_sol_sav_vis_enerbill_page',
            'Screen view',
            'Visibility ',
            'How much wasyour latest monthly energy bill? 2nd step'
          )
        } else if (index == 2) {
          pushDataLayer(
            'exp_intr_sol_sav_vis_utilprov_page',
            'Screen view',
            'Visibility ',
            'Who is your utility provider? 3rd step'
          )
          ...
          pushDataLayer(
            'exp_intr_sol_sav_vis_housloc_page',
            'Screen view',
            'Visibility ',
            'Find yourhouse location 4th step'
          )
        } else if (index == 4) {
          pushDataLayer(
            'exp_intr_sol_sav_vis_roofgetsun_page',
            'Screen view',
            'Visibility ',
            'Does yourroof get sunlight? 5th step'
          )
        } else if (index == 6) {
          pushDataLayer('exp_intr_sol_sav_vis_yourname_page', 'Screen view', 'Visibility ', 'What is your name?')
        } else if (index == 7) {
          pushDataLayer(
            'exp_intr_sol_sav_vis_numbreach_page',
            'Screen view',
            'Visibility ',
            'What is the best number to reach you at if you qualify?'
          )
    ...
    const style = /* HTML */ `
      <style>
        ...
        .content-wrapper::before {
          ...
          position: absolute;
          content: '';
          background: linear-gradient(180deg, #edf2f5 12.81%, rgba(255, 255, 255, 0) 100%),
            url('${git}/sunvalue/img/bgr_img.png');
      ...
      <div class="crs_thank">
        ...
        <div class="container">
          ...
          <ul class="crs_thank_list">
            ...
            <li data-num="2">
              During the call, you will learn if your home qualifies for solar incentives, the suggested solar system
              size, and your predicted solar energy savings
            </li>
          </ul>
        </div>
      </div>
  ...
  handleShowPopup(content) {
    ...
    pushDataLayer(
      'exp_intr_sol_sav_vis_houslocpup_scre',
      'Screen view',
      'Visibility ',
      'Find yourhouse location 4th step Pop up comment '
    )
      ...
      btn.addEventListener('click', e => {
        if (e.currentTarget) {
          if (!e.target.getAttribute('data-test')) {
            ...
            pushDataLayer(
              'exp_intr_sol_sav_but_houslocpup_clos',
              'Close',
              'Button',
              'Find your house location 4th step Pop up comment'
            )
    ...
    backdrop.addEventListener('click', e => {
      if (!e.target.getAttribute('data-test')) {
        if (e.target.matches('.new-popup-backdrop')) {
          ...
          pushDataLayer(
            'exp_intr_sol_sav_but_houslocpup_clos_behind',
            'Close',
    ...
    const mainStyles = /* HTML */ `
      <style>
        ...
        @media (min-width: 1023px) {
          .wrapper::before {
            background:
              linear-gradient(180deg, #edf2f5 12.81%, rgba(255, 255, 255, 0) 100%),
              url(${git}/sunvalue/img/bgr_img.png) bottom no-repeat;
            opacity: 1 !important;
            background-size: cover;
          }
          .wrapper::after {
        ...
        @media (max-width: 768px) {
          .wrapper.show::before {
            background:
              linear-gradient(180deg, #edf2f5 12.81%, rgba(238, 243, 246, 0.93) 23.54%, rgba(255, 255, 255, 0.2) 100%),
              url(${git}sunvalue/img/bgr_img_show_mob.png) no-repeat;
            opacity: 1 !important;
            background-size: cover;
          }
          .wrapper.show::after {
