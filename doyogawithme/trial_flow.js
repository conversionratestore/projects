(function() {
  ...
  class k {
    ...
    on(t, n, i) {
      return typeof n == "function" && (i = n, n = ""), this.elements.forEach(function(p) {
        p.addEventListener(t, function(r) {
          var l;
          if (n !== "") {
  ...
  const e = (a) => new k(a), L = (a) => {
    ...
    if (l = new IntersectionObserver(
      function(s) {
        ...
        }, p) : (f("Element is not fully visible", "warn"), clearTimeout(u));
      },
      { threshold: [r] }
    ), typeof a == "string") {
...
}/*# sourceMappingURL=style.css.map */`, A = `<svg xmlns="http://www.w3.org/2000/svg" width="7" height="12" viewBox="0 0 7 12" fill="none">
...
</svg>`, M = `<svg xmlns="http://www.w3.org/2000/svg" width="181" height="28" viewBox="0 0 181 28" fill="none">
...
</svg>`, g = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="19" viewBox="0 0 20 19" fill="none">
...
</svg>`, C = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
...
`
  ...
  <div class="step1 active">
    ...
    <ul>
      <li><span>${d}</span>1000s of AD-FREE yoga, meditation and movement classes</li>
      <li><span>${d}</span>Exclusive access to all new releases and premium content</li>
      <li><span>${d}</span>Large collection of curated programs & challenges</li>
      <li><span>${d}</span>25% off livestream events</li>
      <li><span>${d}</span>Follow your favorite teachers</li>
      <li><span>${d}</span>Watch on Mobile and TV app in HD video quality</li>
    </ul>
    <button class="start_trial">Start My 7-Day Free Trial</button>
    <p><b>${$}Cancel any time</b> We’ll send you reminder 3 days before your trial ends.</p>
  </div>
`
  ), S = () => {
  ...
  <div class="step2">
    ...
    <ul>
    <li><span>${d}</span>Free 7 day trial. Cancel anytime with access to basic plan.</li>
    <li><span>${d}</span>We’ll remind you  3 days before your trial ends</li>
    <li><span>${d}</span>30-Day Money-Back Guarantee</li>
    </ul>
    ...
    <div class="select_plan">
      <label>
        ...
        <div class="plan_block">
          <p>Yearly</p>
          <p>${a === "CAD" ? "CA$ 12.33" : a === "EUR" ? "€ 8.76" : "$ 9.08"} / mo<span>save ${a === "CAD" ? "CA$80" : a === "EUR" ? "€57" : "$59"}</span></p>
        </div>
      </label>
      <label>
        ...
        <div class="plan_block">
          <p>Monthly</p>
          <p>${a === "CAD" ? "CA$ 18.99" : a === "EUR" ? "€ 13.48" : "$ 13.99"} / mo</p>
        </div>
    </div>
    <p class="total_now">Today you pay:<b>$ 0</b></p>
    <p class="total_after">After 7 days: <span>${a === "CAD" ? "CA$ 147.98" : a === "EUR" ? "€ 105.10" : "$ 108.99"}</span></p>
    <a href='/yogi/register?destination=/express-checkout/139' class="btn to_checkout">Next</a>
  </div>
`
    );
  }, w = (a) => {
    const t = (/* @__PURE__ */ new Date()).getTime(), n = new Date(t + 4 * 24 * 60 * 60 * 1e3).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    }), i = new Date(t + 7 * 24 * 60 * 60 * 1e3).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    });
      ...
      <div class="trial_popup">
        <div class="left_part">
          <div class="steps_wrapper">
            <span class="back ${a ? "hide" : ""}">${A} back</span>
  ...
  `
    ...
    const a = (/* @__PURE__ */ new Date()).getTime(), t = new Date(a + 4 * 24 * 60 * 60 * 1e3).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    }), n = new Date(a + 7 * 24 * 60 * 60 * 1e3).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    });
    ...
    <div class="trial_works">
      ...
      <ul>
        ...
        <li>
          <span>${m}</span>
          <p><b>${n}</b>Your subscription starts unless you cancel it.</p>
        </li>
      </ul>
    </div>
  `
...
`
  ...
  v({ name: "Reverse trial flow", dev: "YK" }), c("reverse_trial", "Start experiment", "loaded"), L("exp_trial_flow");
  class E {
    constructor() {
    ...
    async init() {
      ...
      switch (await o("body"), document.body.insertAdjacentHTML("afterbegin", P), await this.addPopup(), t) {
        case "/":
          this.home();
          break;
      ...
      (t === "/yoga-classes" || t === "/yoga-challenges" || t === "/yoga-programs" || t === "/yoga-meditation" || t === "/asana-labs") && this.listing(), t.includes("/content/") && this.videoPage(), t.includes("/checkout") && this.checkout(), window.location.search.includes("freetrial") && !window.location.search.includes("destination") && this.showPopup(), setTimeout(() => {
        this.setEvents();
      }, 2e3);
    }
    async home() {
      const t = await this.checkAuth();
      this.isAuth = t, this.isAuth || this.unauthorizedPlanUpdate();
    }
    async bas() {
      const t = await this.checkAuth();
      this.isAuth = t, this.isAuth ? this.authorizedPlanUpdate() : this.unauthorizedPlanUpdate();
    }
    async listing() {
      ...
      const t = await this.checkAuth();
      this.isAuth = t, e(".sfc-upgradeBanner__cta").on("click", (n) => {
        if (n.preventDefault(), this.isAuth) {
          e(".popup_wrapper").addClass("active"), c("exp_trial_flow_button_08", "starting free trial", "click", "Listing page");
          return;
        } else
    ...
    async videoPage() {
      ...
      const t = await this.checkAuth();
      this.isAuth = t;
      const n = await o(".sfc-playablePreviewFunnel__overlay--actions h2");
      n.innerHTML.includes("Premium") ? (this.isFree = !1, n.style.display = "none", n.insertAdjacentHTML("afterend", '<h3 class="headline">Start your free trial to unlock this class.</h3>')) : (n.style.display = "none", n.insertAdjacentHTML("afterend", '<h3 class="headline">Create account to start free trial.</h3>')), !this.isFree && this.isAuth && (e(".sfc-playablePreviewOverlayUpsell__upsellLink").style("display", "none"), e(".sfc-playablePreviewOverlayUpsell__upsellLink").elements[0].insertAdjacentHTML("afterend", z), e(".trial_video").on("click", () => {
        this.showPopup(), c("exp_trial_flow_button_07", "starting free 7-day trial", "click", "Premium video");
    ...
    async checkout() {
      ...
      o("div.sfc-orderSummaryItemPrice__price").then((s) => {
        ...
        s.innerHTML = h === "monthly" ? p : i, e(".order-total-line-label").elements[0].innerHTML = "Today you pay", e("del.sfc-orderSummaryItemPrice__price").elements[0].style.display = h === "monthly" ? "none" : "inline", h === "monthly" ? (e(".sfc-orderSummaryItemPrice__highlight").elements[0].style.display = "none", e(".sfc-orderSummaryTotal__savings").elements[0].style.display = "none") : (e(".sfc-orderSummaryItemPrice__highlight").elements[0].innerHTML = `Just ${r}/month!`, e(".sfc-orderSummaryTotal__savings").elements[0].innerHTML = `You save ${l} (35% off)`), e(".order-total-line__total").elements[0].insertAdjacentHTML(
          "afterend",
          `<p class="total_after" style="width: 100%;display: flex;justify-content: space-between;margin: 0;">After 7 days: <span style="color: #A5A5A5;font-weight: 600;">${h === "monthly" ? p : i}</span></p>`
        );
      ...
      }), o(".sfc-registrationProgress__steps").then((s) => {
        s.style.display = "none";
      }), await o(".sfc-registrationProgress__back"), e(".sfc-registrationProgress__back").elements[0].insertAdjacentHTML("afterend", D), e(".sfc-registrationProgress__back").on("click", (s) => {
        s.preventDefault();
        const h = localStorage.getItem("current_page");
        localStorage.removeItem("current_page"), window.location.href = h || "/become-a-subscriber";
      }), window.innerWidth > 768 ? (await o(".layout-region-checkout-derisk"), e(".layout-region-checkout-derisk").elements[0].insertAdjacentHTML("afterend", x())) : e(".layout-region.layout-region-checkout-main").elements[0].insertAdjacentHTML("beforebegin", x());
      const u = await o(".button--primary.js-form-submit");
      u.innerText = "Start my 7-day free trial", u.style.textTransform = "capitalize", u.addEventListener("click", (s) => {
        c("exp_trial_flow_button_05", "Start my 7-day free trial", "click", "Payment information");
      });
    }
    async checkAuth() {
      const t = await o(".c-button--cta.c-dropdownMenu__link");
      return !!(t && (t.innerHTML.includes("Subscribe") || t.innerHTML.includes("Start Free Trial")));
    }
    async unauthorizedPlanUpdate() {
      await o(".sfc-pricingTable__col:nth-child(3) .sfc-pricingTableLevel__pricing--paid");
      const t = e(".sfc-pricingTableLevel__pricing--paid").elements;
      t.length > 1 && t.forEach((n, i) => {
        var r, l;
        const p = ((l = (r = n.querySelector(".sfc-price__amt")) == null ? void 0 : r.textContent) == null ? void 0 : l.trim()) || "";
        n.style.display = "none", n.insertAdjacentHTML("afterend", b(p));
      }), e('.sfc-pricingTable__col [data-sfc-id="button"]').elements.forEach((n, i) => {
        i !== 0 && (n.innerText = "Start Free Trial", n.setAttribute("href", `/yogi/register?destination=/express-checkout/13${i === 1 ? 9 : 8}`));
      });
    }
    async authorizedPlanUpdate() {
      await o(".sfc-pricingTablePremium__col:nth-child(2) .sfc-pricingTableLevel__pricing--paid"), e(".sfc-pricingTableLevel__pricing--paid").elements.forEach((n) => {
        var p, r;
        const i = ((r = (p = n.querySelector(".sfc-price__amt")) == null ? void 0 : p.textContent) == null ? void 0 : r.trim()) || "";
        n.style.display = "none", n.insertAdjacentHTML("afterend", b(i));
      }), e('.sfc-pricingTablePremium__col [data-sfc-id="button"]').elements.forEach((n, i) => {
        n.innerText = "Start Free Trial", n.setAttribute("href", `/yogi/register?destination=/express-checkout/13${i === 0 ? 8 : 9}`);
      });
    }
    async addPopup() {
      const t = await o(".c-button--cta.c-dropdownMenu__link");
      t && t.innerHTML.includes("Subscribe") && (t.innerHTML = "Start Free Trial", t.addEventListener("click", (n) => {
        n.preventDefault(), this.showPopup(), c("exp_trial_flow_button_06", "Start free trial", "click", "Header");
      })), t && t.innerHTML.includes("Sign Up") && t.addEventListener("click", (n) => {
        n.preventDefault(), window.location.href = "/yogi/register?destination=" + window.location.pathname + "?freetrial";
      }), await new Promise((n) => {
        const i = setInterval(() => {
          window.drupalSettings && (clearInterval(i), n(!0));
        }, 500);
      }), window.location.search.includes("freetrial") && window.location.pathname.includes("/content/") && !document.querySelector(".sfc-playablePreviewFunnel__overlay--actions h2") ? document.body.insertAdjacentHTML("beforeend", w(!0)) : document.body.insertAdjacentHTML("beforeend", w()), e(".skip").on("click", () => {
        e(".popup_wrapper").removeClass("active"), c(
        ...
        e(".trial_popup .step1").removeClass("active"), e(".trial_popup .step2").addClass("active"), e(".step_block span:nth-child(2)").addClass("active"), e(".skip") && e(".back").removeClass("hide"), c("exp_trial_flow_button_02", "Start my 7-day free trial", "click", "Pop up start free trial");
      }), e(".trial_popup .back").on("click", () => {
        e(".trial_popup .step1.active").elements.length > 0 ? (e(".popup_wrapper").removeClass("active"), c("exp_trial_flow_button_03", "Back - Step 1", "click", "Pop up start free trial")) : (e(".trial_popup .step2").removeClass("active"), e(".trial_popup .step1").addClass("active"), e(".step_block span:nth-child(2)").removeClass("active"), e(".skip").elements.length > 0 && e(".back").addClass("hide"), c("exp_trial_flow_button_03", "Back - Step 2", "click", "Pop up start free trial"));
      }), e(".trial_popup .select_plan input").on("change", (n) => {
        const i = window.drupalSettings.dywm_commerce_currencies.current_currency, p = i === "CAD" ? "CA$ 147.98" : i === "EUR" ? "€ 105.10" : "$ 108.99", r = i === "CAD" ? "CA$ 18.99" : i === "EUR" ? "€ 13.48" : "$ 13.99";
        n.target.value === "monthly" ? (e(".total_after span").elements[0].innerText = r, e(".to_checkout").elements[0].setAttribute("href", "/yogi/register?destination=/express-checkout/138")) : (e(".total_after span").elements[0].innerText = p, e(".to_checkout").elements[0].setAttribute("href", "/yogi/register?destination=/express-checkout/139"));
      }), e(".to_checkout").on("click", () => {
        const n = e(".trial_popup .select_plan input:checked").elements[0].value;
        localStorage.setItem("current_page", window.location.pathname), c("exp_trial_flow_button_04", `Next - ${n}`, "click", "Pop up start free trial");
      });
    }
    showPopup() {
    ...
    async checkSignup() {
      const t = await this.checkAuth();
      this.isAuth = t;
      const n = localStorage.getItem("signup");
      t && n && !window.location.pathname.includes("/yogi/intake-survey") && (localStorage.removeItem("signup"), window.location.href = n);
    }
    setEvents() {
      _(".step1", "exp_trial_flow_view_01", "Pop up start free trial", "Step 1"), _(".step2", "exp_trial_flow_view_02", "Pop up start free trial", "Step 2"), _(".trial_works", "exp_trial_flow_view_03", "Payment information", "How our trial works"), _(
        ".sfc-upgradeBanner__heading",
        "exp_trial_flow_view_04",
