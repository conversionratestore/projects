  ...
  class _ {
    ...
    on(n, t, i) {
      return typeof t == "function" && (i = t, t = ""), this.elements.forEach((a) => {
        a.addEventListener(n, function(p) {
          var o;
          if (t !== "") {
            let d = (o = p.target) == null ? void 0 : o.closest(t);
            d && (i == null || i.call(d, p));
          } else
            i == null || i.call(a, p);
        });
    ...
    style(n, t) {
      const i = n.split("-").map((a, p) => p === 0 ? a : a.charAt(0).toUpperCase() + a.slice(1)).join("");
      return this.elements.forEach(function(a) {
        a.style[i] = t;
      }), this;
    }
    find(n) {
      const t = this.elements.map((i) => Array.from(i.querySelectorAll(n)));
      return new _(t.flat());
    }
    attr(n, t) {
  ...
  const e = (r) => new _(r), A = (r) => {
    ...
    if (o = new IntersectionObserver(
      function(u) {
        ...
        }, a) : (C("Element is not fully visible", "warn"), clearTimeout(d));
      },
      { threshold: [p] }
    ), typeof r == "string") {
...
}/*# sourceMappingURL=style.css.map */`, S = `<svg xmlns="http://www.w3.org/2000/svg" width="7" height="12" viewBox="0 0 7 12" fill="none">
...
</svg>`, $ = `<svg xmlns="http://www.w3.org/2000/svg" width="181" height="28" viewBox="0 0 181 28" fill="none">
...
</svg>`, b = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="19" viewBox="0 0 20 19" fill="none">
...
</svg>`, w = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
...
</svg>`, x = `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="17" viewBox="0 0 22 17" fill="none">
...
</svg>`, z = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none">
...
`
  ...
  <div class="step1 active">
    ...
    <ul>
      <li><span>${c}</span>1000s of AD-FREE yoga, meditation and movement classes</li>
      <li><span>${c}</span>Exclusive access to all new releases and premium content</li>
      <li><span>${c}</span>Large collection of curated programs & challenges</li>
      <li><span>${c}</span>25% off livestream events</li>
      <li><span>${c}</span>Follow your favorite teachers</li>
      <li><span>${c}</span>Watch on Mobile and TV app in HD video quality</li>
    </ul>
    <button class="start_trial">Start My 7-Day Free Trial</button>
    <p><b>${z}Cancel any time</b> We’ll send you reminder 3 days before your trial ends.</p>
  </div>
`
  ), E = () => {
  ...
  <div class="step2">
    ...
    <ul>
    <li><span>${c}</span>Free 7 day trial. Cancel anytime with access to basic plan.</li>
    <li><span>${c}</span>We’ll remind you  3 days before your trial ends</li>
    <li><span>${c}</span>30-Day Money-Back Guarantee</li>
    </ul>
    ...
    <p class="total_after">After 7 days: <span>${i}</span></p>
...
`
      ...
      <div class="trial_popup">
        <div class="left_part">
          <div class="steps_wrapper">
            <span class="back ${r ? "hide" : ""}">${S} back</span>
            <div class="step_block">
              <span class="active"></span>
              <span></span>
              <span></span>
            </div>
            ${r ? '<span class="skip">skip</span>' : ""}
          </div>
          <div className="content_wrapper">${Z} ${E()}</div>
        </div>
  ...
  `
    ...
    const r = (/* @__PURE__ */ new Date()).getTime(), n = new Date(r + 4 * 24 * 60 * 60 * 1e3).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    }), t = new Date(r + 7 * 24 * 60 * 60 * 1e3).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    });
...
`
  ...
  H({ name: "Reverse trial flow", dev: "YK" }), s("reverse_trial", "Start experiment", "loaded"), A("exp_trial_flow");
  class F {
    constructor() {
    ...
    async init() {
      ...
      switch (await l("body"), document.body.insertAdjacentHTML("afterbegin", V), await this.addPopup(), n) {
        case "/":
          this.home();
          break;
      ...
      (n === "/yoga-classes" || n === "/yoga-challenges" || n === "/yoga-programs" || n === "/yoga-meditation" || n === "/asana-labs") && this.listing(), n.includes("/content/") && this.videoPage(), n.includes("/checkout") && this.checkout(), window.location.search.includes("freetrial") && !window.location.search.includes("destination") && this.showPopup(), setTimeout(() => {
        this.setEvents();
      }, 2e3);
    }
    async home() {
    ...
    async listing() {
      await l(".sfc-upgradeBanner__heading"), e(".sfc-upgradeBanner__heading").elements[0].innerHTML = 'Start Your Free 7-Day Trial for <span style="color:#007922">Unlimited Access</span>', e(".sfc-upgradeBanner__cta").elements[0].innerHTML = "Start Free Trial";
      const n = await this.checkAuth();
      this.isAuth = n, e(".sfc-upgradeBanner__cta").on("click", (t) => {
        if (t.preventDefault(), this.isAuth) {
          e(".popup_wrapper").addClass("active"), s("exp_trial_flow_button_08", "starting free trial", "click", "Listing page");
          return;
        } else
    ...
    async videoPage() {
      ...
      }), await l(".sfc-playablePreviewFunnel__overlay .sfc-playablePreviewFunnel__button--register"), e(".sfc-playablePreviewFunnel__overlay .sfc-playablePreviewFunnel__button--register").on("click", (i) => {
        localStorage.setItem("signup", window.location.pathname + "?freetrial");
      });
    }
    async checkout() {
      ...
      l(".views-field-title .tablesaw-cell-content").then((h) => {
        var T;
        const f = (T = e(".views-field-title .tablesaw-cell-content").elements[0].textContent) != null && T.includes("Monthly") ? "monthly" : "yearly";
        h.innerHTML = f === "monthly" ? p : a, e(".order-total-line-label").elements[0].innerHTML = "Today you pay", e("del.sfc-orderSummaryItemPrice__price").elements[0].style.display = f === "monthly" ? "none" : "inline", f === "monthly" ? (e(".sfc-orderSummaryItemPrice__highlight").elements[0].style.display = "none", e(".sfc-orderSummaryTotal__savings").elements[0].style.display = "none") : (e(".sfc-orderSummaryItemPrice__highlight").elements[0].innerHTML = `Just ${d}/month!`, e(".sfc-orderSummaryTotal__savings").elements[0].innerHTML = `You save ${u} (35% off)`);
        let L = !1;
        e(".coupon-redemption-form__coupons td").elements.forEach((M) => {
          (M.innerHTML.includes("freedom") || M.innerHTML.includes("mindful")) && (L = !0);
        }), e(".order-total-line__total").elements[0].insertAdjacentHTML(
          "afterend",
          `<p class="total_after" style="width: 100%;display: flex;justify-content: space-between;margin: 0;">After ${L ? "3 month" : "7 days"}: <span style="color: #A5A5A5;font-weight: 600;">${f === "monthly" ? p : a}</span></p>`
        );
      }), l(".sfc-registrationProgress__back").then(() => {
        e(".sfc-registrationProgress__back").style("font-size", "14px").style("text-transform", "lowercase");
      }), l(".sfc-registrationProgress__steps").then((h) => {
        h.style.display = "none";
      }), await l(".sfc-registrationProgress__back"), e(".sfc-registrationProgress__back").elements[0].insertAdjacentHTML("afterend", D), e(".sfc-registrationProgress__back").on("click", (h) => {
        h.preventDefault();
        const f = localStorage.getItem("current_page");
        localStorage.removeItem("current_page"), window.location.href = f || "/become-a-subscriber";
      }), window.innerWidth > 768 ? (await l(".layout-region-checkout-derisk"), e(".layout-region-checkout-derisk").elements[0].insertAdjacentHTML("afterend", k())) : e(".layout-region.layout-region-checkout-main").elements[0].insertAdjacentHTML("beforebegin", k());
      const g = await l(".button--primary.js-form-submit");
      g.innerText = "Start my 7-day free trial", g.style.textTransform = "capitalize", g.addEventListener("click", (h) => {
        s("exp_trial_flow_button_05", "Start my 7-day free trial", "click", "Payment information");
      });
    }
    async checkAuth() {
      const n = await l(".c-button--cta.c-dropdownMenu__link");
      return !!(n && (n.innerHTML.includes("Subscribe") || n.innerHTML.includes("Start Free Trial")));
    }
    async unauthorizedPlanUpdate() {
      await l(".sfc-pricingTable__col:nth-child(3) .sfc-pricingTableLevel__pricing--paid");
      const n = e(".sfc-pricingTableLevel__pricing--paid").elements;
      n.length > 1 && n.forEach((t, i) => {
        var p, o;
        const a = ((o = (p = t.querySelector(".sfc-price__amt")) == null ? void 0 : p.textContent) == null ? void 0 : o.trim()) || "";
        t.style.display = "none", t.insertAdjacentHTML("afterend", y(a));
    ...
    async authorizedPlanUpdate() {
      await l(".sfc-pricingTablePremium__col:nth-child(2) .sfc-pricingTableLevel__pricing--paid"), e(".sfc-pricingTableLevel__pricing--paid").elements.forEach((t) => {
        var a, p;
        const i = ((p = (a = t.querySelector(".sfc-price__amt")) == null ? void 0 : a.textContent) == null ? void 0 : p.trim()) || "";
        t.style.display = "none", t.insertAdjacentHTML("afterend", y(i));
    ...
    async addPopup() {
      ...
      n && n.innerHTML.includes("Subscribe") && (n.innerHTML = "Start Free Trial", n.addEventListener("click", (t) => {
        t.preventDefault(), this.showPopup(), s("exp_trial_flow_button_06", "Start free trial", "click", "Header");
        ...
        const i = setInterval(() => {
          window.drupalSettings && (clearInterval(i), t(!0));
        }, 500);
      }), window.location.search.includes("freetrial") && window.location.pathname.includes("/content/") && !document.querySelector(".sfc-playablePreviewFunnel__overlay--actions h2") ? document.body.insertAdjacentHTML("beforeend", v(!0)) : document.body.insertAdjacentHTML("beforeend", v()), e(".skip").on("click", () => {
        e(".popup_wrapper").removeClass("active"), s(
        ...
        e(".trial_popup .step1").removeClass("active"), e(".trial_popup .step2").addClass("active"), e(".step_block span:nth-child(2)").addClass("active"), e(".skip") && e(".back").removeClass("hide"), s("exp_trial_flow_button_02", "Start my 7-day free trial", "click", "Pop up start free trial");
      }), e(".trial_popup .back").on("click", () => {
        e(".trial_popup .step1.active").elements.length > 0 ? (e(".popup_wrapper").removeClass("active"), s("exp_trial_flow_button_03", "Back - Step 1", "click", "Pop up start free trial")) : (e(".trial_popup .step2").removeClass("active"), e(".trial_popup .step1").addClass("active"), e(".step_block span:nth-child(2)").removeClass("active"), e(".skip").elements.length > 0 && e(".back").addClass("hide"), s("exp_trial_flow_button_03", "Back - Step 2", "click", "Pop up start free trial"));
      }), e(".trial_popup .select_plan input").on("change", (t) => {
        const i = window.drupalSettings.dywm_commerce_currencies.plans_pricing, a = window.drupalSettings.dywm_commerce_currencies.current_currency, p = i[1].prices[a], o = i[0].prices[a];
        t.target.value === "monthly" ? (e(".total_after span").elements[0].innerText = o, e(".to_checkout").elements[0].setAttribute("href", "/yogi/register?destination=/express-checkout/138")) : (e(".total_after span").elements[0].innerText = p, e(".to_checkout").elements[0].setAttribute("href", "/yogi/register?destination=/express-checkout/139"));
      }), e(".to_checkout").on("click", () => {
        const t = e(".trial_popup .select_plan input:checked").elements[0].value;
        localStorage.setItem("current_page", window.location.pathname), s("exp_trial_flow_button_04", `Next - ${t}`, "click", "Pop up start free trial");
      });
    }
    showPopup() {
    ...
    setEvents() {
      m(".step1", "exp_trial_flow_view_01", "Pop up start free trial", "Step 1"), m(".step2", "exp_trial_flow_view_02", "Pop up start free trial", "Step 2"), m(".trial_works", "exp_trial_flow_view_03", "Payment information", "How our trial works"), m(
