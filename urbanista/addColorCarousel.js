...
document.querySelector('.product-info-price').insertAdjacentHTML(
    ...
    `   
        <div class="color-wrapper">
            ...
            <div class="color-wrapper--opacity"></div>
        </div>
    `,
);
...
const observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutationRecord) {
        let color = document.querySelector('.product-info-main').style
            .backgroundColor;

        document.querySelector(
            '.color-wrapper--opacity',
        ).style.background = `linear-gradient(to left, ${color}, rgba(0, 0, 0, 0))`;
    });
});
