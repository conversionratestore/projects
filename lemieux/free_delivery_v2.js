;(function () {
  ...
  const C = (i, e, r, n = '') => {
    ...
    u = i => document.querySelectorAll(i),
    t = i => document.querySelector(i),
    m = i => {
      let e = setInterval(function () {
        typeof window.clarity == 'function' && (clearInterval(e), window.clarity('set', i, 'variant_1'))
      }, 1e3)
    },
    d = (i, e, r, n, a = 3e3, l = 0.5) => {
      let s, c
      if (
        ((s = new IntersectionObserver(
          function (o) {
            o[0].isIntersecting === !0
              ? (c = setTimeout(() => {
                  C(e, o[0].target.dataset.visible || n || '', 'visibility', r), s.disconnect()
                }, a))
              : clearTimeout(c)
          },
          { threshold: [l] }
  ...
  function h(i) {
    return new Promise(e => {
      if (document.querySelector(i)) return e(document.querySelector(i))
...
`
  m('exp_free_delivery'), y({ name: 'Free Delivery Threshold_2nd iteration', dev: 'SKh' })
  const V = window.location.pathname,
    M = window.innerWidth < 768 ? 'mobile' : 'desktop'
  class P {
    constructor(e, r) {
      ;(this.observer = null),
    ...
    init() {
      h('body.content-initiated').then(() => {
        this.visibleHandler(),
          ...
          document.head.insertAdjacentHTML('afterbegin', `<style>${L}</style>`)
      })
    }
    observePageChange() {
        ...
        r.forEach(n => {
          window.location.pathname !== this.lastPath &&
            ...
            window.location.pathname === '/'
              ...
              : document.body.classList.contains('is_homepage') && document.body.classList.remove('is_homepage')
          for (let a of n.addedNodes)
            a instanceof HTMLElement &&
              ((a.matches('basket-view-totals > .flex') || a.matches('modal-bottom-container basket-add-notice')) &&
                (this.getGrandTotalPriceFromWindow(),
                this.cartPage(),
                this.productPage(),
                this.addEventListeners(),
                this.device === 'desktop' ? this.checkBasketDesktop() : this.checkBasketMobile()),
              a.matches('[cy-basketempty]') &&
                localStorage.getItem('basketPriceForIcon') &&
                (localStorage.removeItem('basketPriceForIcon'), this.changeIconHeader(), this.productPage()))
        })
      })
    ...
    renderFreeDeliveryBanner() {
      ...
      switch ((console.log(r), r)) {
        case 'product':
          ;(n = t('header .listing_var.free_delivery_banner')) == null || n.remove(),
            this.device === 'desktop' ? (e = 'beforebegin') : (e = 'afterend'),
            t('header .free_delivery_banner') ||
              ((a = t('#header > .page-header__inner')) == null ||
                a.insertAdjacentHTML(e, f(this.currency, this.threshold)),
              t('header .free_delivery_banner') && t('header .free_delivery_banner').classList.add('product_var'),
              d(
          ...
          break
        case 'listing':
          ;(l = t('header .product_var.free_delivery_banner')) == null || l.remove(),
            (e = 'afterend'),
            t('header .free_delivery_banner') ||
              ((s = t('#header > .page-header__inner')) == null ||
                s.insertAdjacentHTML(e, f(this.currency, this.threshold)),
              t('header .free_delivery_banner') && t('header .free_delivery_banner').classList.add('listing_var'),
              d(
    ...
    productPage(e = !1) {
      setTimeout(() => {
        var c
        let r = 0,
          n = t('product-view-delivery-note'),
          a = t('product-view-price .price'),
          l = localStorage.getItem('basketPriceForIcon')
        const s = l !== null
        if (
          (e &&
            ((c = t('product-view-layout .threshold_banner')) == null || c.remove(), console.log('remove >>>>>>>>>.')),
          n && ((n.style.display = 'none'), a))
        ) {
          let o = this.priceToNumber(a.textContent)
          if (o !== r && ((r = o), s && o <= +l && (o = +l), !t('product-view-layout .threshold_banner'))) {
            n.insertAdjacentHTML('beforebegin', this.drawThreshold(o))
            let b = setInterval(() => {
              t('.cdk-overlay-backdrop') ||
                (clearInterval(b),
                d(
    ...
    checkBasketMobile() {
      this.device === 'mobile' &&
        h('basket-add-notice .b-t').then(e => {
          var r
          t('basket-add-notice .free_delivery_banner') ||
            ((r = t('basket-add-notice .b-t')) == null ||
              r.insertAdjacentHTML('beforebegin', f(this.currency, this.threshold)),
            d(
    ...
    checkBasketDesktop() {
      h('minibasket basket-view-totals').then(e => {
        t('minibasket .custom-scrollbar>div:first-child') &&
          (t('minibasket .custom-scrollbar>div:first-child').style.display = 'none')
        let r = localStorage.getItem('basketPriceForIcon')
        r !== null &&
          ((r = +r),
          t('minibasket .threshold_banner') && t('minibasket .threshold_banner').remove(),
          t('minibasket basket-view-totals').insertAdjacentHTML('afterend', this.drawThreshold(r, !0)),
          d(
    ...
    cartPage() {
      h('basket-view-totals>div:first-of-type>div').then(e => {
        ...
        if (this.checkPage() === 'basket') {
          ...
          n !== null &&
            ((n = +n),
            t('basket-view .threshold_banner') && t('basket-view .threshold_banner').remove(),
            this.device === 'mobile'
              ? t('basket-view bottom-panel').insertAdjacentHTML('afterbegin', this.drawThreshold(n))
    ...
    changeIconHeader() {
      let e = !1
      const r = localStorage.getItem('basketPriceForIcon'),
        n = r !== null && +r >= this.threshold
      u('basket-qty').forEach(a => {
        var c, o, b
        const l = !!a.querySelector('.header_icon'),
          s = a.querySelector('[cy-basketbagbtn]')
        if (n) {
          if (!l) {
            if (e) return
            ...
            v &&
              this.priceToNumber(v.textContent) < this.threshold &&
              ((c = t('product-view-layout .threshold_banner')) == null || c.remove())
            let g = setInterval(() => {
              t('.cdk-overlay-backdrop') ||
                (clearInterval(g),
                d(
          ...
          if (a.querySelector('.header_icon') == null) return
          ;(e = !1), (o = a.querySelector('.header_icon')) == null || o.remove()
          let v = t('product-view-price .price')
          v &&
            this.priceToNumber(v.textContent) < this.threshold &&
            ((b = t('product-view-layout .threshold_banner')) == null || b.remove()),
    ...
    getGrandTotalPriceFromWindow() {
      ...
      const e =
        (s =
          (l =
            (a =
              (n = (r = window == null ? void 0 : window.app) == null ? void 0 : r.customer) == null
                ? void 0
                : n.basket) == null
              ? void 0
              : a.data) == null
    ...
    drawThreshold(e, r = !1) {
      ...
      if (e < this.threshold) return H(n, this.currency, this.threshold, e, this.country)
      if (e >= this.threshold) return x(this.country)
    }
    visibleHandler() {
      h('basket-qty .page-header__icon').then(e => {
        d('basket-qty .page-header__icon', 'exp_free_delivery_icon_vis_01', 'Icone', 'Header Cart icon visibility')
      })
    }
    addEventListeners() {
      h('header .header_icon').then(e => {
        var r
        ;(r = t('header .header_icon')) == null ||
          r.addEventListener('click', n => {
            n.target.getAttribute('data-test') ||
              C('exp_free_delivery_icon_01', 'Icone', 'click', 'Header Free delivery icon  visibility'),
              n.target.setAttribute('data-test', '1'),
              setTimeout(() => {
