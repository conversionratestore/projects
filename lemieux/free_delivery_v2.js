...
`
  ...
  const L = window.location.pathname, V = window.innerWidth < 768 ? "mobile" : "desktop", M = (
    /*HTML */
    `
<style>
  ...
  class P {
    ...
    init() {
      h("body.content-initiated").then(() => {
        this.visibleHandler(), this.getGrandTotalPriceFromWindow(), this.renderFreeDeliveryBanner(), this.productPage(), this.cartPage(), this.addEventListeners(), document.head.insertAdjacentHTML("afterbegin", M);
      });
    }
    observePageChange() {
    ...
    changeIconHeader() {
      ...
      u("basket-qty").forEach((a) => {
        ...
        if (n) {
          ...
          let v = t("product-view-price .price");
          v && this.priceToNumber(v.textContent) < this.threshold && ((b = t("product-view-layout .threshold_banner")) == null || b.remove()), s.style.display = "block";
          let g = setInterval(() => {
            t(".cdk-overlay-backdrop") || (clearInterval(g), d(
