  ...
  if (document.querySelector("#menu-list")) {
    ...
    function pushDataLayer(actionDataLayer, labelDataLayer) {
      ...
      if (labelDataLayer) {
        ...
        dataLayer.push({
          event: "event-to-ga",
          eventCategory: `Exp: Autofill ZIP City ${eventVar}`,
    ...
    let stickyBlock = /*html */ `
      ...
      <section class="sticky_banner">
          ...
          <label>
              ...
              <span class="auto_city var_sticky"></span>
          </label>
...
<div class="auto_location_block">
    ...
    <label>
        ...
        <input type="number" autocomplete="off" pattern="[0-9]*" name="zipCode"  placeholder="Your zip code" class="">
        <span class="auto_city var_auto_loc"></span>
    </label>
    <span class="zip_error">Write your zip code</span>
    <button>Compare quotes</button>
</div>
`
    ...
    if (document.querySelector("section .css-1qsp2gq form.css-8atqhb")) {
      onAutoFillLocation()
    }
      ...
      function onLoadStickyBanner() {
        ...
        if (document.querySelector(".sticky_banner")) {
          if (
            ...
            (window.location.pathname !== "/insurance/auto/car-insurance-estimate-calculator/" &&
              document.querySelector(".banner-container")?.getBoundingClientRect().bottom <= positionVar)
          ) {
              ...
              if (document.querySelector(".sticky_banner")) {
                  ...
                  btnSend.addEventListener("click", (e) => {
                    ...
                    if (!e.target.getAttribute("data-test")) {
                      ...
                      if (document.querySelector(".chakra-form__error-message.css-vamxt0") !== null) {
                        ...
                        if (document.querySelector(".auto_city svg")) {
                          document.querySelectorAll(".auto_city").forEach((e) => {
                            e.innerHTML = ""
                          })
                        }
                      }
                ...
                if (input) {
                  ...
                  input.addEventListener("input", (e) => {
                    ...
                    document.querySelector(".auto_location_block input").value = e.target.value
                  })
    ...
    function onAutoFillLocation() {
      document.querySelector("section .css-1qsp2gq form.css-8atqhb").insertAdjacentHTML("afterbegin", autoLocationBlock)

      if (document.querySelector(".auto_city.var_auto_loc")) {
        if (document.querySelector(".auto_city.var_auto_loc") !== "") {
          const options = {
            root: null,
            threshold: 1,
          ...
          let observerNewHeader = new IntersectionObserver((entries) => {
            if (!entries[0].isIntersecting) return
            pushDataLayer(`City resolved from IP and shown`)
            observerNewHeader.disconnect()
          })

          observerNewHeader.observe(document.querySelector(".auto_city.var_auto_loc"), options)
        }
      }
          ...
          if (btnSend) {
            btnSend.addEventListener("click", (e) => {
              ...
              if (!e.target.getAttribute("data-test")) {
                ...
                document.querySelector("section .css-1qsp2gq form.css-8atqhb button.chakra-button").click()
        ...
        .then((jsonResponse) => {
          ...
          if (jsonResponse.postal) {
            if (document.querySelector('section .css-1qsp2gq .css-8atqhb [name="zip"]')) {
              ...
              nativeInputValueSetter.call(document.querySelector('section .css-1qsp2gq .css-8atqhb [name="zip"]'), jsonResponse.postal)
              var ev2 = new Event("input", { bubbles: true })
              document.querySelector('section .css-1qsp2gq .css-8atqhb [name="zip"]').dispatchEvent(ev2)

              if (document.querySelector(".auto_city.var_auto_loc")) {
                console.log(`first`, document.querySelector(".auto_city.var_auto_loc"))
                document.querySelector(".auto_location_block input").value = jsonResponse.postal
                document.querySelector(".auto_city.var_auto_loc").textContent = jsonResponse.city
                arr.push({
                  code: jsonResponse.postal,
                  name: jsonResponse.city,
                })
                localStorage.setItem("auto_city", JSON.stringify(arr))
              }
            }
            ...
            let a = setInterval(() => {
              if (document.querySelector(".sticky_banner input")) {
                ...
                document.querySelector(".auto_city.var_sticky").textContent = jsonResponse.city
                if (!localStorage.getItem("auto_city")) {
                  arr.push({
                    code: jsonResponse.postal,
                    name: jsonResponse.city,
                  })
    ...
    function onInput(e, error, label) {
      ...
      nativeInputValueSetter.call(document.querySelector('section .css-1qsp2gq .css-8atqhb [name="zip"]'), e.target.value)
      ...
      document.querySelectorAll(".auto_city").forEach((i) => {
      ...
      if (localStorage.getItem("auto_city")) {
        temp = JSON.parse(localStorage.getItem("auto_city"))
        temp.forEach((item) => {
    ...
    const record = setInterval(() => {
      if (typeof clarity === "function") {
        ...
        clarity("set", `autofill_zip_city_${eventVar}`, "variant_1")
      }
    }, 200)
  }
}, 700)
