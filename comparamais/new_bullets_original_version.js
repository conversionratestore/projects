...
if (settings.clarity) {
  const clarityInterval = setInterval(function () {
    if (typeof clarity == 'function') {
      ...
      try {
        clarity('set', 'new_bullets', 'original');
      } catch (error) {
        console.log(error);
      }
    }
...
let startFunk = setInterval(() => {
  if (document.querySelector('#hlApp')) {
    ...
    if (!location.search.includes('experiment=HL-Simulator-New_Bullets-Original')) {
      if (location.search) {
        window.history.replaceState(
          null,
          null,
          location.search + '&experiment=HL-Simulator-New_Bullets-Original'
        );
      } else {
        window.history.replaceState(
          null,
          null,
          '?experiment=HL-Simulator-New_Bullets-Original'
        );
      }
    }
