(function() {
  ...
  const w = async (e, n) => {
    const t = e, o = (n == null ? void 0 : n.getBoundingClientRect().top) + window.pageYOffset - t;
...
<path d="M3 13.7002L8 18.7002L13 13.7002" stroke="#FFE401" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  `
    ...
    mainBenefits: [
      {
        svg: `${c.benefitIconFirst}`,
        txt: "All natural content & chemical free"
      },
      {
        svg: `${c.benefitIconSecond}`,
...
header.hidden {
  ...
  class L {
    ...
    addEventsAccordion() {
      s(".lp-tr--what-tick-accordion .card .btn").then((t) => {
      });
    ...
    renderBtnGetZenToAccordionBody() {
      s(".lp-tr--what-tick-accordion .card-body").then((n) => {
        p(".lp-tr--what-tick-accordion .card-body").forEach((i) => {
