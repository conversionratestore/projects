...
let selectPackage = `
    <div class="select-kids d-flex d-md-none justify-content-between">
        <label class="mb-0">
...
for (let i = 0; i < packs.length; i++) {
    ...
    newElementPacksLabel.innerHTML = `
        ...
        <span class="row align-items-center packs-block">
            ...
            <span class="col-4 px-0 text-left"><span class="fw-bold c-pink packs-days" data-days="${packs[i].days}">${packs[i].days} days</span><span class="for-people"></span></span>
        </span>
    `;  
...
function counterIf() {
    ...
    if (document.querySelector('.counter-input').value <= 1) {
        ...
        document.querySelectorAll('.packs-days').forEach((el) => {
            el.innerHTML = `${(el.dataset.days / document.querySelector('.counter-input').value).toFixed(1)} days`;
        });
    } 
}
