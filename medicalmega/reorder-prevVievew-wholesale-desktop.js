...
function resData(data) {
    if (mm.userId != 0) {
        ...
        if (data["items"].length > 4 && document.querySelector('.ordered .view-more') != null) {
...
function pushDataLayer(action,label) {
    ...
    dataLayer.push({
        'event': 'event-to-ga',
        'eventCategory': 'Exp: Wholesale easy reorder desktop',
        'eventAction': `${action}`,
        'eventLabel': `${label}`
    });
}
...
let style = `
            ...
            .add-to-cart {
                ...
                margin: 8px 0 0;
                width: 100%;}
            .add-to-cart button:focus, .btn-reorder:focus {
...
hj('event', 'wholesale_easy_reorder_desktop');
window.dataLayer = window.dataLayer || [];
dataLayer.push({
    'event': 'event-to-ga',
    'eventCategory': 'Exp: Wholesale easy reorder desktop',
