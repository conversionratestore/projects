if (window.location.pathname.includes('blogs')) {
    ...
    let start = setInterval(function () {
        if(document.querySelectorAll('.shg-btn-wrapper')[1]) {
            clearInterval(start)
            mainPage()
        }
    }, 100)
...
function blogPage() {
    ...
    const banner = `
        <div class="blog_banner">
           ...
           <div>
           <p class="title">How good is your sleep?</p>
    ...
    document.querySelector('.to_quiz2').addEventListener('click', function () {
        ...
        dataLayer.push({
            'event': 'event-to-ga',
            'eventCategory': 'Exp — Quiz hp and blog pages (mobile)',
...
function mainPage() {
    const style = `
    <style>
        #s-b4e509bc-83be-4723-bbbf-02b6e7f40e88-btn-wrapper {
          display: flex;
          align-items: center;
        }
        
        #s-b4e509bc-83be-4723-bbbf-02b6e7f40e88-btn-wrapper>a:first-child {
          flex-shrink: 0;
        }
...
`
    ...
    const mainLink = `
    <a href="https://somnifix.com/pages/quiz-11" class="to_quiz1">Discover Your Sleep Quality</a>
`
    document.body.insertAdjacentHTML('afterbegin', style)
    document.querySelectorAll('.shg-btn-wrapper')[1].insertAdjacentHTML('beforeend', mainLink)
