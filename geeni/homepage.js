import {startLog, waitForElement, loadScriptsOrStyles, clarityInterval, pushDataLayer, visibilityOfTime } from '../libraries/index.ts';

// -------------------------------------
// CONSTANTS
// -------------------------------------
const WAIT_INTERVAL_TIMEOUT = 100
const dir = 'https://conversionratestore.github.io/projects/geeni/img/home-app-web'
...
const html: string = /*html*/`
  <div class="additional-functional">
    <div data-img-func="accessibility" >
      <img src="${dir}/accessible.svg" alt="Open accessibility features">
    </div>
    <div data-img-func="sale" class="hidden-el">
      <img src="${dir}/sale.svg" alt="Open sale proposition">
    </div>
    <div data-img-func="chat">
      <img src="${dir}/chat.svg" alt="Open suppport chat">
      <div><p></p></div>
    </div>
  </div>

  <div class="free-shipping">
    <img src="${dir}/delivery.svg" alt="shipping">
    ...
    <div class="container">
      <div class="shop-all__heading">
        ...
        <a href="/collections/all" class="shop-all__view">
          View all
          <img src="${dir}/arrow-r.svg" alt="">
        </a>
      </div>
      <div class="menu-items">
        <a href="/collections/smart-security-cameras">
          <img src="${dir}/cameras.jpg" alt="Cameras">
          <p>Cameras</p>
        </a>
        <a href="/collections/smart-appliances">
          <img src="${dir}/lifestyle.jpg" alt="Lifestyle">
          <p>Lifestyle</p>
        </a>
        <a href="/collections/lighting">
          <img src="${dir}/lighting.jpg" alt="Lighting">
          <p>Lighting</p>
        </a>
        <a href="/collections/power">
          <img src="${dir}/power.jpg" alt="Power">
          <p>Power</p>
        </a>
      </div>
    </div>
  ...
  <section class="many-devices">
    <div class="container">
      <div class="many-devices__split">
        <img src="${dir}/house-1.jpg" alt="">
        <img src="${dir}/house-2.jpg" alt="">
      </div>
      <div class="many-devices__mobiles">
        <img src="${dir}/many-devices.svg" alt="">
      </div>
  ...
  <section class="trusted-reviews">
    <div class="container">
      ...
      <div class="trusted-reviews__rate">
        <img src="${dir}/4-stars.svg" alt="4 stars">
      ...
      <div class="trusted-load-more load-more-btn-visible">
          ...
          <div class="swiper-slide">
            <img src="${dir}/5-stars.svg" alt="5 stars">
...
// MAKE DOM CHANGES
// -------------------------------------
clarityInterval("exp_home_optim")
...
waitForElement('#shopify-section-template--16970486120700__section-marquee').then(el => el.insertAdjacentHTML('afterend', html))
addFont()
handleClicksOnAdditionalFunctional() 
initSlider()
moveMenuOnMobile()
waitForElement('body').then(el => el?.classList.add('hide-chat-btn'))
waitForElement('iframe[title="Messaging window"]').then(iframe => {
  const observer = new MutationObserver((mutationsList) => {
    for(let mutation of mutationsList) {
      if (mutation.type === 'attributes' && mutation.attributeName === 'tabindex') {
        console.log('Tabindex changed to:', iframe.getAttribute('tabindex'));
...
insertProductsIntoMySection()
waitForElement('.load-more-btn').then(el => {
...
waitForElement('body').then(body => {
  visibilityOfTime('.free-shipping', 'exp_home_optim_section_01', 'Home Page Free Shipping on Orders Over $69', 'Section', 4000)
  
  visibilityOfTime('.shop-all .container', 'exp_home_optim_section_02', 'Home Page Shop by category', 'Section')

  visibilityOfTime('.hero .swiper', 'exp_home_optim_section_03', 'Home Page Featured products PDP', 'Section')

  visibilityOfTime('.warranty-sale', 'exp_home_optim_section_04', 'Home Page Benefits', 'Section')

  body.addEventListener('click', (e) => {
    const target = e.target as HTMLElement;
  
    if (target.closest('button.mobile-menu__button')) {
      pushDataLayer('exp_home_optim_button_01', 'Menu', 'Button', 'Home Page Header');
    }
    if (target.closest('.shop-all__view')) {
      pushDataLayer('exp_home_optim_button_01', 'Menu', 'Button', 'Home Page Header');
    }
    if (target.closest('.menu-items a')) {
      const label= target.closest('.menu-items a')?.querySelector('p')
      pushDataLayer('exp_home_optim_button_05', `${label?.innerText} - choise category`, 'Button', 'Home Page Shop by category');
    }
    if (target.closest('.hero .product__btn')) {
      const label= target.closest('.product')?.querySelector('.product__title')

      pushDataLayer('exp_home_optim_button_06', `${label?.innerText} - Shop now`, 'Button', 'Home Page Featured products PDP');
    }
    if (target.closest('.hero .product')) {
      const label= target.closest('.hero .product')?.querySelector('.product__type')

      pushDataLayer('exp_home_optim_button_07', `${label?.innerText} - Click`, 'Button', 'Home Page Featured products PDP');
    }
  })
})
...
function fetchProductData(handle: string, index: number): Promise<Product> {
  return fetch(`https://mygeeni.com/products/${handle}.json`, {
    method: 'GET',
    headers: {
  ...
  .then(data => {
    ...
    return {
      ...
      description: (document.querySelectorAll('.products-list__item__description')[index] as HTMLElement)?.innerText
    };
  });
}

function drawHeroSection() {
  const dealsProductsHandles: string[] = [];

  const topProductsHandle = [
  ...
  document.querySelectorAll<HTMLAnchorElement>('.products-list__container .btn.btn--primary').forEach(el => {
    const href = el.href; // use href property to get full URL
    if (href) {
      dealsProductsHandles.push(href.split('https://mygeeni.com/products/')[1]); // use push method to add href to the array
    }  
  });

  const arrivalRequest = dealsProductsHandles.map((handle, index) => fetchProductData(handle, index));
  Promise.all(arrivalRequest)
  .then(products => {
        ...
        <div class="glide__slide new-arrivals-product">
            ...
            <div class="new-arrivals-product__content">
              <div class="new-arrivals__nav">
                ...
                <p class="new-arrivals-product__number">${index + 1}/${productsLength}</p>
                <div class="swiper-button-next-mob">
                  <img src="${dir}/arrow-r.svg" alt="arrow right">
                </div>
              </div>
              <a class="new-arrivals-product__title" href="/products/${product.handle}">${product.title}</a>
              <div class="new-arrivals-product__price">
                <span>$${product.price}</span>
              </div>
              <p class="new-arrivals-product__description">${(document.querySelectorAll<HTMLElement>('.products-list__item__description')[index]).innerText}</p>
              <a class="crs-btn" href="/products/${product.handle}">Shop now</a>
            </div>
    ...
    // Insert the hero string into the DOM
    waitForElement('.new-arrivals .new-arrivals__content').then(el => el?.insertAdjacentHTML('beforeend', arrival))
  })
  .catch(error => console.error('Error:', error));

  const handles = window._dy_customer_logged_in && window._dy_customer_logged_in.email !== undefined ? dealsProductsHandles : topProductsHandle;

  const requests = handles.map((handle, index) => fetchProductData(handle, index));
  ...
  Promise.all(requests)
    .then((products: Product[]) => {
      ...
      const slides = products.map((product, index) => {
        ...
        const savingsMessage = savings > 0 ? `Save ${savingsPercent}%` : '';
        const compareAtPriceMessage = product.compare_at_price && product.compare_at_price !== product.price ? `$${product.compare_at_price}` : '';
        const productType = product.product_type.toLowerCase().includes('camera') ? 'Cameras' : product.product_type;

        return /*html*/`
          <div class="swiper-slide product">
            <img class="product__img" src="${product.image_src}" alt="${product.title}">
            <div class="product__number">
              <p>${index+1}/${products.length}</p>
            </div>
            <div class="product__info">
              <p class="product__type">${productType}</p>
              <a class="product__title" href="/products/${product.handle}">${product.title}</a>
              <div class="product__price">
                <span>$${product.price}</span>
                <span>${compareAtPriceMessage}</span>
                ${savingsMessage !== '' ? `<span>${savingsMessage}</span>` : ''}
              </div>
  
              <a class="product__btn" href="/products/${product.handle}">Shop now</a>
            </div>
          </div>`;
      }).join('');
  
      const hero: string = /*html*/`
        <div class="hero">
          <div class="container">
      ...
      // Insert the hero string into the DOM
      if (device === 'desktop') {
        waitForElement('.free-shipping').then(el => el?.insertAdjacentHTML('afterend', hero))
      } else {
        waitForElement('.shop-all').then(el => el?.insertAdjacentHTML('afterend', hero))
      }
    })
    .catch(error => console.error('Error:', error));
}
...
function addFont() {
  function addLink(href: string, rel: string, crossorigin?: string) {
  ...
  addLink('https://fonts.googleapis.com/css2?family=Manrope:wght@200..800&display=swap', 'stylesheet');
}
    ...
    if(accessibilityPluginBtn && chatPlugin) {
      ...
      waitForElement('.additional-functional').then(el => el.addEventListener('click', (e) => {
        ...
        if (e.target.closest('[data-img-func="accessibility"]')) {
          ...
          (document.querySelector('.kl-teaser-T4UsqC') as HTMLElement)?.click();
      ...
      setInterval(() => {
        ...
        if (document.querySelector('iframe[title="Number of unread messages"]')) {
          const chatPluginMsg = document.querySelector('iframe[title="Number of unread messages"]')
          ...
          if (document.querySelector('[data-img-func="chat"] p') && chatMsgNumber?.textContent) {
            ...
            console.log(chatMsgNumber.textContent);
...
function initSlider() {
  loadScriptsOrStyles(['https://cdn.jsdelivr.net/npm/swiper@10/swiper-bundle.min.js',  'https://cdn.jsdelivr.net/npm/swiper@10/swiper-bundle.min.css'])

  const waitForSlider = setInterval(() => {
    if (
      typeof Swiper !== 'undefined'
      // && document.querySelectorAll('.swiper')[2]
    ) {
      clearInterval(waitForSlider)

      waitForElement('.hero .swiper').then(el => {
  ...
  on: {
    ...
    slideChange: function () {
      ...
      waitForElement('.trusted-reviews .swiper').then(el => {
...
function moveMenuOnMobile() {
  if (device === 'mobile') {
    waitForElement('.mobile-menu [aria-controls="nav-drawer"]').then(el => {
      document.querySelector('h1.logo').insertAdjacentElement('beforebegin', el)
    })  
  }  
}

function insertProductsIntoSection(sectionType, productSelector) {
  const sectionSelector = `[data-products-type="${sectionType}"] .products-wrapper`;

  waitForElement(sectionSelector).then(el => {
    const productGridItems = document.querySelectorAll(productSelector);
    for (let i = 0; i < 4; i++) {
      const productGridItem = productGridItems[i];
      el.appendChild(productGridItem);
    }

    const waitForEl = setInterval(() => {
      const productTitles = document.querySelectorAll(`${sectionSelector} .product-grid-item__title`);
      if(productTitles[3]) {
        clearInterval(waitForEl)
    
        productTitles.forEach((el, index) => {
          document.querySelectorAll(`${sectionSelector} .product-grid-item__inner`)[index].insertAdjacentHTML('beforeend', `<a class="product__shop-now" href="${el.href}">Shop now</a>`)
        })
      }
    }, WAIT_INTERVAL_TIMEOUT)
  })
...
function insertProductsIntoMySection() {
  const waitForEl = setInterval(() => {
    if(document.querySelector('#shopify-section-template--16970486120700__c779e8b3-bda2-49eb-b1f7-031ddd03321b .product-grid-item__inner')
      && document.querySelector('#shopify-section-template--16970486120700__section_collection_CbegNw .product-grid-item__inner')
    ) {
      clearInterval(waitForEl)
  
      insertProductsIntoSection('popular-products', '#shopify-section-template--16970486120700__c779e8b3-bda2-49eb-b1f7-031ddd03321b .product-grid-item__inner');
      insertProductsIntoSection('hot-deals', '#shopify-section-template--16970486120700__section_collection_CbegNw .product-grid-item__inner');
    }
  }, WAIT_INTERVAL_TIMEOUT)
}
