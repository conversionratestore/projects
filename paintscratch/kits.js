...
const itemsData = {
  ...
  nonMetalSpayPaintKit: {
    title: 'Non-Metal Spray Paint Kit',
...
class Kits {
  ...
  #eventsList() {
    ...
    $$el('.kit_item_image a').forEach(item => {
      item.addEventListener('click', event => {
        pushDataLayer('epx_kits_select_selectouch_icone', 'Icone', 'Select', 'Select Your Products Touch Up Paint Kits')
      })
    })
    $$el('.kit_item_content a').forEach(item => {
      item.addEventListener('click', event => {
        pushDataLayer('epx_kits_select_selectouch_titlprod', 'Product title', 'Select', 'Select Your Products Touch Up Paint Kits')
      })
    })
  ...
  #eventsPdp() {
    blockVisibility('.product-images-popup table', 3, 'Visibility', 'PDP')
  }
  #modifyKitListHeader() {
  ...
  #modifyKitPdpItem() {
    ...
    if (this.device === 'Mobile') {
      $el('.product-images-popup table tr:nth-of-type(2)').before($el('.product-images-popup table tr:nth-of-type(3)'))
    }
  }
  #addChipsToItem() {
    if (this.targetPage === 'list') {
      $$el('.kit-item').forEach(item => {
        for (let key in itemsData) {
          if (item.querySelector('h2').textContent.trim() === itemsData[key].title) {
    ...
    if (this.targetPage === 'item') {
      ...
      for (let key in itemsData) {
        if (kitItemTitle.trim() === itemsData[key].pdpTitle) {
          const chipsContainer = kitItemContainer.querySelector('.kit_item_chips')
          itemsData[key].chips.forEach((chip, i) => {
            let chipClass = 'kit_item_chip'
            if (i === 0) {
              chipClass += ' kit_item_chip-highlighted'
