(function() {
  ...
  }, s = "https://conversionratestore.github.io/projects/paintscratch/img/", c = {
    zoom: (
      /*html */
      `
  ...
  <div class="d-lg-flex crs_images_row">
    <div class="crs_images_left relative"><img src="${s}paint-code-locations/mazda.png" alt="image">${c.zoom}</div>
    <div class="crs_images_right">
      <div class="relative"><img src="${s}color-id-tag/Hyundai-Paint-Code.gif" alt="image">${c.zoom}</div>
    </div>
  </div>
</div>
<button type="button" class="crs_video" style="display: none">${c.play} <span>Video instruction on how to find your color code</span></button>
<h3>Select the paint color based only on your color code:</h3>`
  ...
  <div class="crs_popup_container">
    <div class="crs_popup_head">Video instruction ${c.close}</div> 
  ...
  <div class="crs_popup_container">
    <div class="crs_popup_head">${c.close}</div> 
    <img src="#" alt="image">
  </div> 
</div>`
  ), C = `#page {
...
@media screen and (max-width: 768px) {
  ...
  class x {
    ...
    init() {
      e(".crs_style_color") || document.head.insertAdjacentHTML("beforeend", `<style class="crs_style_color">${C}</style>`), this.addAccordion(), this.addImages(), this.addVideo(), this.closePopup(), this.device === "mobile" && this.addReadMore();
    }
    addAccordion() {
      e(".crs_accordion") || e(".select-color span.instructions + p") && (e(".select-color span.instructions + p").insertAdjacentHTML("afterend", f), e(".crs_accordion_current").addEventListener("click", (n) => {
        ...
        let o = "";
        e(".crs_accordion").classList.contains("active") ? o = "Open" : o = "Close", a("exp_color_code_button_03", o, "Button", "What is a color code?");
      }));
    }
    addImages() {
      if (e("#color-display-table") && !e(".crs_images")) {
        ...
        for (let n in t)
          if (e("#page #main h1").innerText.toLowerCase().includes(n.toLowerCase())) {
            let o = t[n][2] != null ? `<div class="relative"><img src="${s}color-id-tag/${t[n][2]}" alt="image">${c.zoom}</div>` : "";
            e(".crs_images_left img").src.includes(t[n][0]) || (e(".crs_images_left img").src = s + "paint-code-locations/" + t[n][0]), e(".crs_images_right").innerHTML = `<div class="relative"><img src="${s}color-id-tag/${t[n][1]}" alt="image">${c.zoom}</div>` + o;
          }
    ...
    addVideo() {
      ...
      let n = e(".crs_video");
      for (let o in g)
        e("#page #main h1").innerText.toLowerCase().includes(o.toLowerCase()) && (n.insertAdjacentHTML("afterend", y(g[o])), e(".crs_popup_video iframe").addEventListener("click", (r) => {
          a("exp_color_code_button_01", "Play", "Button", "Video instruction pop up");
        }), n.style.display = "flex", n.addEventListener("click", (r) => {
    ...
    addReadMore() {
      d("#main h3").forEach((n) => {
        if (n.innerText.includes("Color Code Information") && !n.nextElementSibling.querySelector(".crs_read_more")) {
          const o = n.nextElementSibling, r = o.innerText, l = h(r);
          o.innerHTML = l, e(".crs_read_more").addEventListener("click", (b) => {
            o.innerHTML = r, a("exp_color_code_link_02", "More", "Link", "Color Code Information");
          });
        }
      });
    }
