...
let initHeader = setInterval(() => {
    if (  document.querySelector('header .search_header input') != null && document.querySelector('.header_dropdown .city_wr') != null) {
        ...
        document.querySelector('#close_ic_desktop').addEventListener('click', () => {
            document.querySelector('.search_header').classList.remove('visible_search')
        })

        //dropdown
        //add menu right
        document.querySelector('.header_dropdown .main_container').insertAdjacentHTML('afterbegin', `
        <div class="header_dropdown_top">
            <div class="container">
                <ul class="right-menu d-flex">
                    <li><a href="/blocg">Blog</a></li>
        ...
        document.querySelector('.header_dropdown .right-menu').before(document.querySelector('.country_wr'))

        //add popular tour block
        document.querySelector('.header_dropdown .city_wr').insertAdjacentHTML('afterend',`
                ...
                if (item.classList.contains(key)) {
                    for (let i = 0; i < objPopularTours[key].length; i++) {
                        if (i < 4) {
                            ...
                            item.insertAdjacentHTML('beforeend', popularTout(tour.image, tour.rate, tour.name, tour.link))
                        }
                    }
                }
            })
        ...
        document.querySelector('.main_menu .droped').addEventListener('click', (e) => {
            e.target.closest('header').parentElement.classList.toggle('destinations-active');
        })

        document.querySelectorAll('.header_dropdown .block .heading div').forEach((item, index) => {
            item.innerHTML = `
...
let init = setInterval(() => {
    if ((document.querySelector('#plugin iframe') != null || window.matchMedia("(max-width: 767px)").matches) && document.querySelector('header .main_menu') != null) {
        clearInterval(init)
        ...
        let resizeLeft = () => {
            ...
            document.querySelector('#plugin').style = `left: ${tourLeft.offsetLeft + tourLeft.clientWidth}px; top: ${tourLeft.offsetTop}px;`
        }
        resizeLeft()
        ...
        if (window.matchMedia("(max-width: 767px)").matches) {
            document.querySelector('.descr-section').insertAdjacentHTML('afterend', footerRightHTML)

            document.querySelector('.descr-section').after(document.querySelector('.awards-desktop'))
        } else {
        ...
        for (let i = 0; i < descrObj.length; i++) {
            ...
            if (document.querySelector(`.destination_page_wr .destination_descr .${icon}`) != null) {
                ...
                document.querySelector(`.left .descr-section .row`).insertAdjacentHTML('beforeend',`
        ...
        async function initExp() {
            ...
            if (rateEl) {
                ...
                document.querySelector('.rating').insertAdjacentHTML('beforeend', `<div class="star-row d-flex align-items-center">
                ...
                let rate = document.querySelector('.rating .rate-tours').innerHTML.replace(/[^\d\.]*/g, '');
                let ratehalf = rate.split('.')[1]
                ...
                if (ratehalf > 0) {
                    document.querySelector('.rating .star-row').insertAdjacentHTML('afterbegin', starHalf) 
                    document.querySelector('.rating_header').insertAdjacentHTML('afterbegin', starHalf) 
                }
                for (let i = 0; i < Math.floor(rate); i++) {
                    document.querySelector('.rating .star-row').insertAdjacentHTML('afterbegin', star) 
            ...
            if (reviewsEl && scriptEl) {
                ...
                if (customers_reviews) {
                    ...
                    for (let i = 0; i < customers_reviews.length; i++) {
                        ...
                        if (document.querySelector('.slider-review .swiper-wrapper') != null) {
        ...
        async function fetchReviews() {
            let requestUrl = document.querySelector('.ret_back')?.href || location.href.match(/^(.*?\/\/[^\/]+\/[^\/]+\/).*$/)[1];
            ...
            document.querySelectorAll('.grey_block').forEach(item => {
                ...
                if (item.querySelector('a') == null) {
                    if (document.querySelector('.meeting_spot > iframe') != null) {
                        document.querySelector('.grey_block').after(document.querySelector('.meeting_spot > iframe'))
                        document.querySelector('.meeting-spot iframe').style.marginTop = '16px'
                    } 
                }
            })
        } else {
...
let otherTours = setInterval(() => {
    if (document.querySelector('.other-tours') != null && (document.querySelector('.country_tours') != null || document.querySelector('.similar_tours') != null)) {
        ...
        document.querySelectorAll('.other-tours .cardx-container-details').forEach(item => {
            if (item.querySelector('.cardx-price') != null && item.querySelector('.cardx-price').innerHTML != '') {
            ...
            if (item.querySelector('.rate-tours') != null) {
                let rate = item.querySelector('.rate-tours').innerHTML.replace(/[^\d\.]*/g, '');
                let ratehalf = rate.split('.')[1]
                item.querySelector('.rate-tours').innerHTML = rate;

                for (let i = 0; i < Math.floor(rate); i++) {
                    item.querySelector('.star-ratings .rate-tours').insertAdjacentHTML('beforebegin', isStar) 
                }
                if (ratehalf > 0) {
                    item.querySelector('.star-ratings .rate-tours').insertAdjacentHTML('beforebegin', isStarHalf) 
                }
        ...
        document.querySelectorAll('.other-tours .cardx-tags > span > img').forEach(item => {
