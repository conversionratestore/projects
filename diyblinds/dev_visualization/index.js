...
(function() {
  ...
  const v = (e, s, o, i = "") => {
    ...
    let _ = new IntersectionObserver(
      (r) => {
        r.forEach((t) => {
          t.isIntersecting && (_.unobserve(t.target), setTimeout(function() {
            b.observe(t.target);
          }, 1e3 * n));
        });
      },
      ...
      r.forEach((t) => {
        t.isIntersecting ? (v(
          s || `view_element_${t.target.id}`,
          o || `View element on screen (${n} sec or more)`,
          "view",
          i || t.target.id
        ), d && d(), _.unobserve(t.target)) : _.observe(t.target), b.unobserve(t.target);
      });
    });
  ...
  function h(e) {
    return new Promise((s) => {
      ...
      const o = new MutationObserver(() => {
        document.querySelector(e) && (s(document.querySelector(e)), o.disconnect());
      });
  ...
  const f = (e) => document.querySelectorAll(e), l = (e) => document.querySelector(e), B = (e) => {
    let s = setInterval(function() {
      typeof window.clarity == "function" && (clearInterval(s), window.clarity("set", e, "variant_1"));
    }, 1e3);
  }, y = ({ url: e, fullSize: s = !1, opacity: o = 1 }) => s ? (
    /* HTML */
    `<svg
      width="1345"
      height="1021"
  ...
  ], k = "https://conversionratestore.github.io/projects/diyblinds", I = [
  ...
  class x {
    ...
    render() {
      ...
      <div class="crs-window__fill">${y({ url: `${k}/img/textures/qube_cross.jpg` })}</div>
      <div class="crs-window__image"></div>
    </div>`
      );
      f(".configurator__step").forEach((o) => {
        var i;
        if (I.some((n) => o.textContent.includes(n))) {
          const n = o.querySelector(".row.small-gutters.row-gutters");
          (i = n == null ? void 0 : n.closest(".configurator__step")) == null || i.classList.add("crs-window"), f(".crs-window__show").forEach((_) => {
            _.remove();
          }), n == null || n.insertAdjacentHTML("beforebegin", s);
          const d = setInterval(() => {
            const _ = l(".roller .cart-title__image");
            _ && (_.innerHTML = s, clearInterval(d));
          }, 10);
          o.addEventListener("click", (_) => {
            var t;
            const r = _.target.closest("button.selectable-item");
            if (r) {
              const c = (t = r.querySelector("h4")) == null ? void 0 : t.innerHTML, u = c == null ? void 0 : c.replace(/<br>| |\//g, "_").toLocaleLowerCase().trim(), a = $.find(({ name: w }) => w === u);
              this.allowEvent, f(".crs-window__fill").forEach((w) => {
                w.innerHTML = y({
                  url: `${k}/img/textures/${u}.jpg`,
                  opacity: (a == null ? void 0 : a.opacity) || 1,
                  fullSize: (a == null ? void 0 : a.fullSize) || !1
                });
              });
            }
          });
  ...
  `
    ...
    mobile: "mobile",
    desktop: "desktop"
  }, p = "https://conversionratestore.github.io/projects/diyblinds";
  class C {
    constructor() {
    ...
    init() {
      ...
      f("h3").forEach((r) => {
        var t;
        (t = r.textContent) != null && t.includes("Choose your Roller Blind") && (r.innerHTML = /* HTML */
        `<span class="round-number" style="background: #E83B68;">1</span>
          <span>Select Roller Blind type</span>`, r.insertAdjacentHTML(
          "beforebegin",
          /* HTML */
      ...
      }), f(".configurator__step [data-carousel] .selectable-item").forEach((r) => {
        var t, c, u;
        (t = r.textContent) != null && t.includes("Light Filtering") && r.classList.add("crs-light-filtering"), (c = r.textContent) != null && c.includes("Blockout") && r.classList.add("crs-blockout"), (u = r.textContent) != null && u.includes("Sunscreen") && r.classList.add("crs-sunscreen");
      });
      const s = (
        /* HTML */
        ` <div class="crs-measure">
      <span class="icon">${g.measure}</span>
      ...
      (d = l(".crs-measure")) == null || d.remove(), (_ = l(".measure-panel__howto")) == null || _.insertAdjacentHTML("beforeend", s), f(".step-desc").forEach((r) => {
        var t;
        (t = r.textContent) != null && t.includes("Min/max width and drop") && (r.innerHTML = "Min/max width and drop will vary based on selected fabric range in the previous step.<br> *Motorised options available for Blinds  wider than 595mm.");
      }), h(".colour-chooser__wrap li").then(() => {
        var t, c;
        const r = (
          /* HTML */
          '<div class="crs-select-color">Select your colour</div>'
        );
        (t = l(".crs-select-color")) == null || t.remove(), (c = l(".colour-chooser__wrap")) == null || c.insertAdjacentHTML("beforebegin", r), f(".colour-chooser__wrap li").forEach((u) => {
          var a;
          (a = u.textContent) != null && a.includes("Colours") && (u.querySelector("span").style.backgroundImage = `url(${p}/img/others_color.png)`, u.querySelector("span").style.backgroundSize = "100%");
        });
      }), h(".cart-title__text").then((r) => {
        var c;
        const t = (
          /* HTML */
          `<div class="crs-badges">
        <img src="${p}/img/au_made.png" alt="Made in Australia" />
        <img src="${p}/img/made_by_9.png" alt="Made by 9 working day" />
        <img src="${p}/img/warranty.png" alt="5 year warranty" />
      </div>`
        );
        (c = l(".crs-badges")) == null || c.remove(), r.insertAdjacentHTML("beforeend", t);
      });
      const o = (
        /* HTML */
      ...
      if (l(".roller-container"))
        return;
      (b = l(".crs-window + div")) == null || b.insertAdjacentHTML("beforeend", o);
      const i = l(".o-sidebar__panel-content .row > div");
      i && (i.classList.add("roller"), l(".roller-container").append(i));
  ...
  class S {
    constructor() {
      this.handleClick = (s) => {
        v("exp_visualization_link_01", "Show more fabrics", "click", "PDP Choose 'Blockout' colour & fabric");
        const o = l(".crs-window .card-grid");
        o.classList.toggle("show-more");
        const i = s.target;
        o.classList.contains("show-more") ? (i.innerHTML = "Show less fabrics <span>" + g.arrowDown + "</span>", console.log(i)) : (i.scrollIntoView({ behavior: "smooth" }), i.innerHTML = "Show more fabrics <span>" + g.arrowDown + "</span>");
      };
    }
    render() {
      const s = (
      ...
      <div class="crs-show-more">
        <button class="show-more">Show more fabrics <span>${g.arrowDown}</span></button>
      </div>
    `
      );
      h(".crs-window .card-grid li").then(() => {
        var n, d;
        const o = l(".crs-show-more .show-more");
        o && (o.removeEventListener("click", this.handleClick), (n = o.parentElement) == null || n.remove()), f(".crs-window .card-grid li").length > 10 && ((d = l(".crs-window .row.small-gutters.row-gutters>div:last-of-type")) == null || d.insertAdjacentHTML(
          "beforeend",
          s
  ...
  class T {
    ...
    init() {
      this.isCarouseiIsShown || h("div[data-carousel]").then(() => {
        m(
          "div[data-carousel]",
      ...
      }), this.isMeasureIsShown || h(".crs-measure").then((s) => {
        m(
          ".crs-measure",
      ...
      }), this.isColorIsShown || h(".colour-chooser__wrap li").then(() => {
        m(
      ...
      }), this.isWhiteFabricsIsShown || h(".crs-window .small-gutters .card-grid").then(() => {
        m(
  ...
  q({ name: "Dev Visualization", dev: "OS" }), B("exp_visualization");
  class H {
    constructor() {
      this.allowEvent = !0, this.events = new T();
    }
    init() {
      h(".configurator").then(() => {
        new x().render(), new C().init(), new S().render();
      }), this.events.init(), document.addEventListener("click", (s) => {
        var b, r, t, c, u;
        const o = s.target, i = o.closest("button.selectable-item");
        if (i && ((b = i.textContent) != null && b.includes("Blockout Roller Blind") || (r = i.textContent) != null && r.includes("Light Filtering Roller Blind") || (t = i.textContent) != null && t.includes("Sunscreen Roller Blind"))) {
          h(".crs-window button.selectable-item").then(() => {
            v("exp_visualization_icone_01", "Click", "click", "PDP Select Roller Blind type");
          }), new x().render(), new C().init(), this.events.init();
          const a = setInterval(() => {
            const w = l(".crs-window button.selectable-item:first-child");
            w && (this.allowEvent = !1, w.click(), clearInterval(a), this.allowEvent = !0);
          }, 50);
        }
        const n = o.closest(".colour-chooser__wrap li");
        if (n) {
          const a = (c = n.querySelector(".sr-only")) == null ? void 0 : c.textContent;
          v("exp_visualization_button_01", `${a}`, "click", "PDP Choose 'Blockout' colour & fabric"), new S().render();
        }
        const d = o.closest(".crs-window button.selectable-item");
        if (d) {
          const a = (u = d.querySelector("h4")) == null ? void 0 : u.innerHTML, w = a == null ? void 0 : a.replace(/<br>| |\//g, "_").toLocaleLowerCase().trim();
          this.allowEvent && v("exp_visualization_button_02", `${w}`, "click", "PDP Choose 'Blockout' colour & fabric");
        }
        o.closest(".crs-measure .link") && v("exp_visualization_link_02", "Check guide", "click", "PDP Measurements How to measure?");
      });
    }
  }
  new H().init();
